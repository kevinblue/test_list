 ...
 *
 * @author Ben Alex
 * @version $Id: SessionRegistryImpl.java 3631 2009-05-01 06:45:34Z ltaylor $
 */
public class SessionRegistryImpl implements SessionRegistry, ApplicationListener<SessionDestroyedEvent> {
    ...
    public void removeSessionByUserId(String userId) {
    	try {
			for(String sessionId : this.sessionIds.keySet()){
				 SessionInformation info = getSessionInformation(sessionId);
    ...
    public int getLoginedUserCountByUserId(String userId){
    	int count = 0;
		for(String sessionId : this.sessionIds.keySet()){
				 SessionInformation info = getSessionInformation(sessionId);
				 User user = (User)info.getPrincipal();
