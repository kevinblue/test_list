...
public class PoiExcelUtil {
	...
	public static Workbook readWorkbook(String filepath, ExcelVersionEnum excelVersionEnum) throws Exception {
		...
		try {
			fs=new FileInputStream(StrTools.fileNameCheck(filepath));
			switch (excelVersionEnum) {
			case VERSION2003: {
			...
			if(fs!=null){FileUtil.safeCloseInputStream(fs);}
		}
		return wb;
	}
	...
	// 拷贝当前行的合并单元格
    public static void copyRowMergeRegion(Sheet sheet, CellRangeAddress address,int rowindex) {
    	...
    	for (int i = 0; i < sheetMergeCount; i++) {
		     ...
		     if(range.getFirstColumn()==address.getFirstColumn()&& range.getLastColumn()==address.getLastColumn()){
	     ...
	     if(flag==0){
	     CellRangeAddress cellRangeAddress = new CellRangeAddress(rowindex, rowindex, address.getFirstColumn(), address.getLastColumn());
	     sheet.addMergedRegion(cellRangeAddress);
	     }
	}
	// 强制刷新sheet中的公式
		...
		for (int index = (n - 1); index >= 0; index--) {
			...
			for (int i = mergeRegion.getFirstRow(); i <= mergeRegion.getLastRow(); i++) {
				...
				for (int j = mergeRegion.getFirstColumn(); j <= mergeRegion.getLastColumn(); j++) {
					...
					if (StringUtil.nullToString(cell.getStringCellValue()).trim().equals("")) {
    ...
    //判断是否是合并单元格
	//row格元格行，column单元格列 rowindex要拷贝行，falg是否拷贝1为拷贝
	private static boolean isMergedRegion(Sheet sheet,int row ,int column,int rowindex,int falg) {
		  int sheetMergeCount = sheet.getNumMergedRegions();
		  for (int i = 0; i < sheetMergeCount; i++) {
		     CellRangeAddress range = sheet.getMergedRegion(i);
		     int firstColumn = range.getFirstColumn();
		     ...
		     if(row >= firstRow && row <= lastRow){
		       if(column >= firstColumn && column <= lastColumn){
		    	if(falg==1){
		    		copyRowMergeRegion(sheet,range,rowindex);
		    	}
		         return true;
		        }
		  ...
		  try {
			  ...
			  for (int i = 0; i < rows; i++) {
			   ...
			   for (m = sourceRow.getFirstCellNum(); m <= sourceRow.getLastCellNum(); m++) {
				   ...
				   if(null!=sourceCell){
				   isMergedRegion(sheet,sourceCell.getRowIndex(),sourceCell.getColumnIndex(),targetCell.getRowIndex(),1);
			       if(null!=sourceCell){
	...
	public static List getMergedRegionIndex(Sheet sheet,int rowindex){
		...
		for (m = sourceRow.getFirstCellNum(); m <= sourceRow.getLastCellNum(); m++) {
			   ...
			   for (int i = 0; i < sheetMergeCount; i++) {
				    ...
				    if(lastRow==firstRow&&firstRow==rowindex){
				    	
				    	if(m>firstColumn&&m<=lastColumn){
				    		index=-1;
				    	}
				    }
			  }
	...
	public static ExeclBean getExcelBean(String refence) throws Exception {

		ExeclBean execl = null;
		try {
			execl = new ExeclBean();
			execl.setSheet_name(refence.substring(0, refence.indexOf("!")));
			execl.setCol_name(refence.substring(refence.indexOf("!$") + 2, refence.indexOf("$", refence.indexOf("!$") + 2)));
			execl.setRow_name(refence.substring(refence.indexOf("$", refence.indexOf("!$") + 2) + 1));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			throw new BusinessException("解析" + refence + "出错:" + e.getMessage());
		}
		return execl;
	}
	...
	public static void CopySheet(Sheet fromsheet, Sheet toSheet) throws Exception {
		...
		try {
			...
			XSSFSheet temp=(XSSFSheet)fromsheet;
			for (POIXMLDocumentPart dr : temp.getRelations()) {  
	            if (dr instanceof XSSFDrawing) {  
	                XSSFDrawing ddrawing = (XSSFDrawing) dr;  
	                List<XSSFShape> shapes = ddrawing.getShapes();  
			...
			pict.resize(1);// 该方法只支持JPEG 和 PNG后缀文件
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
