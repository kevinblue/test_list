...
public class MessageTag extends HtmlEscapingAwareTag {
	...
	protected String resolveMessage() throws JspException, NoSuchMessageException {
		...
		//modify by tracywindy updated 2013-08-21
		//LocaleResolver localeResolver = WebUtil.getLocaleResolver();
		//Assert.notNull(localeResolver);
		//Locale locale =  WebUtil.getSessionLocale();//localeResolver.resolveLocale((HttpServletRequest)this.pageContext.getRequest());
		//modify by zhanc updated 2015-03-25
		User user = SecurityUtil.getPrincipal();
		if(user != null){
			Locale locale = null;
			if(WebUtil.getUserSessions().get(user.getId())==null){
				return resolvedText;
			}else{
				try{
					locale = (Locale)WebUtil.getUserSessions().get(user.getId());
				}catch(Exception e){
					return resolvedText;
				}
			}
