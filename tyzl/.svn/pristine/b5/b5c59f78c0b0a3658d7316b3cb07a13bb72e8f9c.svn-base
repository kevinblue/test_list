...
public class ContractOnhireServiceImpl implements ContractOnhireService {
	...
	public void updateContractOnhire(Map<String, String> variablesMap)throws Exception {
		...
		contractInfo.setContractStatus(AppStaticUtil.CONTRACT_START);
		if(null!= contractInfo.getSupContractId()){
			ContractInfo superContract= this.baseService.findEntityByID(ContractInfo.class, contractInfo.getSupContractId());
			superContract.setContractStatus(AppStaticUtil.CONTRACT_START);
			this.baseService.saveOrUpdateEntity(superContract);
		}
		//保存租赁物件
