...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
			...
			params:{adjustid:'<mini:param  name="fund_standard_interest.id" />',docid:flowUnid},
		    columns:[
					{type:'indexcolumn'}, 
					{type:'checkcolumn'},
					...
					{header:'投放编号',field:'contractputnumber',width:150,queryConfig:{}},
					...
					{header:'利率',field:'yearrate',width:60},
...
function todoTX(rowDatas){
	if(confirm("确认对选中合同进行调息?")){
		...
		for(var i=0;i<rowDatas.length&&isContinue;i++){
			showMsg(rowDatas[i]["contractputnumber"]+"开始请求调息!");
			doSendTX(rowDatas[i]["id"],rowDatas[i]["contractputnumber"]);//调用调息ajax
		}
	}
}
