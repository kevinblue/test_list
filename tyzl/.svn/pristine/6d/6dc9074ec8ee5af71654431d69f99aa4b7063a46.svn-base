...
public class EvenCorpusCaleServiceImpl implements RentCalcService {
	...
	public Hashtable<String, Object> rentCal(ConditionBean cb, String calType, String userName, String[] rentAdjustList, int startList,List<FundPlanBean> funds,List<String> ...planList) throws Exception {
		...
		frpb = rentPlanCalByConditionAndTab(cb, tcb, startList, frpb);
		...
		rpfcs.rentPlanCalByConditionAndTab(cb, tcb, startList, frpb);// 添加财务租金计划

		//计算保险费
		String insureMoney = InsureTypeTools.getInsureMoney(cb, frpb);
		cb.setInsureMoney(insureMoney.toString());*/
		...
		//计算实行额
		//BigDecimal result = GrossProfitCalculator.reckonConditionSXE(cb, frpb);
		//cb.setCleanLeaseMoney(result.toString());
		
		//计算粗利
		//BigDecimal grossProfit = GrossProfitCalculator.reckonGrossProfit(cb, frpb, fundPlanList, PBOCInfo);
		...
		re_ht = RentCalculateHelper.createFundPlanCashIrr(cb, tcb,funds, frpb);
	...
	public FundRentPlanBean rentPlanCalByConditionAndTab(ConditionBean cb, TabCalBean tcb, int startList, FundRentPlanBean frpb) throws Exception {
		...
		// 调用保存的操作
		rpcd.addRentPlan(tcb.getRentPlan_tb(), frpb, tcb, startList);// 保存新的租金计划
	...
	private void getPmtRentPlan(ConditionBean cb, FundRentPlanBean frpb) {
		...
		try {
			...
			List<RentPlanInfo> rentPlanList = RentCalculateUtil.calculateForSameCorpus(condition,frpb.getCorpusBusinessList());
	...
	public Hashtable<String, Object> rentCalOnHire(ConditionBean cb,
			...
			List<FundPlanBean> fundPlanList,List<SpecialRulesBean> srb) throws Exception {
		logger.info("进入方法:rentCalOnHire 进行起租时的租金的测算..");
		// 起租时只重新保存交易结构信息，更改租金计划支付日期，更改现金流明细
