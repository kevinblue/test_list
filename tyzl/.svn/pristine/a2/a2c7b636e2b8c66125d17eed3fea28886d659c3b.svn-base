(function (b) {
    ...
    a.rect = function (p, m) {
        ...
        b([t.node, f.node, n.node, e.node]).bind("click", function (e) {
        	var nodeType=  (n.node.string||($(n.node.childNodes[0]).text())).replace(/(<<)|(>>)/gi,"");
            if (!a.config.editable) {
            	if(window.globalWorkflowNodeClickCallBack){var nodeName = f.node.string||($(f.node.childNodes[0]).text());var posX = parseInt(t.X||(t.node.getAttribute("x")));var posY = parseInt(t.Y||(t.node.getAttribute("y")));var nodeWidth=parseInt(t.W||(t.node.getAttribute("width")));var nodeHeight = parseInt(t.H||(t.node.getAttribute("height")));window.globalWorkflowNodeClickCallBack(e,nodeType,nodeName,posX,posY,nodeWidth,nodeHeight);}
                return
            }
            w();
            var o = b(C).data("mod");
            ...
            /*if(("start"!=nodeType)&&("end"!=nodeType)){
               jQuery("#nodeRemoveBtn").show();
            }else{
               jQuery("#nodeRemoveBtn").hide();
            }*/
            jQuery("#nodeRemoveBtn").show();
            return false
        });
    ...
    a.init = function (x, r) {
        var v = b(window).width(),
            e = b(window).height(),
            y = Raphael(x,diagramCanvasWidth,diagramCanvasHeight /*v * 1.5, e * 1.5*/),
            q = {},
            g = {};
        b.extend(true, a.config, r);
	   	var removeBtnClickFunc = function(e){
			 e.stopPropagation();
	         var j = b(y).data("currNode");
	         var jsonObj = eval("("+j.toJson()+")");
	         var nodeType = jsonObj.type;
	         var nodeName = jsonObj.text.text;
	         //if(("start"!=nodeType)&&("end"!=nodeType)){
		         if(window.confirm("确认删除节点《 "+nodeName+" 》么？")){
	        	     jQuery("#nodeRemoveBtn").hide();
	                 if (j) {
        ...
        b(document).keydown(function (e) {
            ...
            if (e.keyCode == 46) {
            	removeBtnClickFunc(e);
            }
        });
        if (a.config.editable) {
        	 //jQuery("#nodeRemoveBtnClick").click(removeBtnClickFunc);
        	 jQuery("#nodeRemoveBtnClick").parent().click(removeBtnClickFunc);
        }
        ...
        b(document).click(function (e) {
        	...
        	if(window.hideGlobalWorkflowNodeDetailWindow){
    			window.hideGlobalWorkflowNodeDetailWindow();
    		}
            ...
            b("#myflow_tools .state").each(function () {
                b(this).draggable({
                    ...
                    drag   :function(e,ui){
                    	...
                    	jQuery("#info").css({"width":"200px"});
                    	jQuery("#info").val("offsetY : "+offsetY+",positionY ： "+positionY);*/
                    }
                })
            });
            b(x).droppable({
                ...
                drop: function (c, i) {
                    b(y).trigger("addrect", [i.helper.attr("type"), {
                        attr: {
                            x: i.helper.offset().left + jQuery("#myflowContainer")[0].scrollLeft,
                            y: i.helper.offset().top  + jQuery("#myflowContainer")[0].scrollTop - 40
                        }
                    }])
                }
            });
        ...
        if (typeof(A.rects) != 'undefined' && typeof(l.rects) != 'undefined' && (A.rects.length || l.rects.length)) {
            var m = {},
                z = {};
            for (var h in g) {
                if (!z[g[h].from().text()]) {
