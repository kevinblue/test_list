...
function checkFundPut(){
	  ...
	  ajaxRequest({
			url : getRootPath() + "/eleasing/getFundPutCheckInfo.acl",
			method : 'POST',
			success : function(rs) {
                ...
                if(checkInfo.result=="0"){
                	 ...
                	 var handlingchargemoney=$minigetinputtext("handlingchargemoney");
                	 var managementmoney=$minigetinputtext("managementmoney");
                	 var cautionmoney=$minigetinputtext("cautionmoney");
                	 if(parseFloat(checkInfo.fund[0].handling_charge_money)<handlingchargemoney){
                		 mini.alert("服务费不能大于剩余投放的服务费！！！");
                		 flag=false;
                	 }
                	 if(parseFloat(checkInfo.fund[0].management_money)<managementmoney){
                		 mini.alert("管理费不能大于剩余投放的管理费！！！");
                		 flag=false;
                	 }
                	 if(parseFloat(checkInfo.fund[0].caution_money)<cautionmoney){
                		 mini.alert("风险金不能大于剩余投放的风险金！！！");
                		 flag=false;
                	 }
                }
