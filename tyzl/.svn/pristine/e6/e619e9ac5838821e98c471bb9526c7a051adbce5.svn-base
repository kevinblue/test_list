...
import com.reckon.calc.EqRatioCorCalc;
import com.tenwa.business.model.RentInfoBox;


public class EqualRatioCorpusUtil {
...
//		String Other_expenditure = conditionBean.getNominalprice();// 残值收入
		...
		String type=conditionBean.getPeriodType();//期末
		String firstMoney= String.valueOf("-"+conditionBean.getCleanLeaseMoney());
		//现金流部分调用
		//装租金测算条件 9个
		EqRatioCorCalc calc =new EqRatioCorCalc();
		calc.setYear_rate(conditionBean.getYearRate()); // 年利率
		calc.setIncome_number(conditionBean.getIncomeNumber()+"");// 期数
		calc.setLease_money(conditionBean.getCleanLeaseMoney());// 租赁本金 
		// - 首付款）
...
//		rentInfoBox.setL_RentInshare(l_rentInshareBean);
		/*for(int i=0;i<l_RentDetails.size();i++){
			...
			System.out.println(
//					date_list.get(i)+"  rent="+rent_list.get(i)+"  cor="+corpus_market.get(i)
//					+"  inte="+inter_list.get(i)+"  cor_ovg="+corpusOverge_market.get(i));
//					"  Details="+l_RentDetails.get(i)
			a.getPlan_date()+"  "+a.getFollow_in()+"  "+a.getFollow_in_detail()+"  "+
			a.getFollow_out()+"  "+a.getFollow_out_detail()+"  "+a.getNet_follow()
