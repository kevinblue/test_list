...
System.out.println("回滚流程结束把调息记录临时表FUND_ADJUST_INTEREST_C_TEMP 的数据写入 FUND_ADJUST_INTEREST_CONTRACT表 docid="+docid);		
	...
	public void updateCopyFundRentPlanFromHisToTemp(Map<String, String> model) throws Exception {
		...
		for(int i=0;i<jsonArray.length();i++){
			...
			String condition="mod_status='his_status_before' and mod_reason='his_rate_change' and doc_id='"+his2docId+"'  and contract_id='"+his2ContractId+"' ";
			List excludeColumn=new ArrayList();
			excludeColumn.add("mod_status");
			excludeColumn.add("mod_reason");
			excludeColumn.add("HIS_ID");
			this.tableService.hisEntityByProcedures("contract_fund_rent_plan_his", "contract_fund_rent_plan_temp", condition, null, excludeColumn);
		/*
			if(!his2.isEmpty()){
...
System.out.println("央行记录临时表删除转化前：contractid="+contractid+"docid="+docid);
	 ...
	 * @return
	 */
	private String getSql(String docid){
		//公用部分SQL抽出
		String publicSonSql = "  where doc_id = '"+docid+"'  ";
		publicSonSql = publicSonSql + "and rent_list = CONTRACT_FUND_RENT_PLAN.Rent_List   ";
		...
		publicSonSql = publicSonSql + "    and t2.id = CONTRACT_FUND_RENT_PLAN.CONTRACT_ID ";
		publicSonSql = publicSonSql + ") ";
		
		StringBuffer sql = new StringBuffer();
		sql.append(" update CONTRACT_FUND_RENT_PLAN set ")
		.append(" rent = ( ")//租金
	...
	public void updateCopyFundRentPlanFromTempToHisBeforeAndAfter(Map<String, String> model) throws Exception {
		...
		this.tableService.getBussinessDao().getJdbcTemplate().update(delSql,docid,"his_status_before",modReason,docid);
		...
		this.tableService.getBussinessDao().getJdbcTemplate().update(sql, docid);
		
		//--------------------------------------------------------------------------------------------------------------------
		//移动数据前先删除
		delSql = this.getDelHisSql(docid,"his_status_after",modReason);
		log.info("调息流程结束时，租金正式表的租金计划移往HIS表前删除:"+delSql);
		this.tableService.getBussinessDao().getJdbcTemplate().update(delSql,docid,"his_status_after",modReason,docid);
		...
		this.tableService.getBussinessDao().getJdbcTemplate().update(sql, docid,docid);
...
//		this.tableService.getBussinessDao().getJdbcTemplate().update(sql.toString(),docid,docid);
//		
	}
	...
	public void deleteHisAllSqlOfTx(String docid) throws DataAccessException, Exception{
		...
		this.tableService.getBussinessDao().getJdbcTemplate().update(delSql,docid);
	}
	...
	public void deleteHisAllSqlOfTx(String docId,String contractId) throws DataAccessException, Exception{
		String delSql = " delete contract_fund_rent_plan_his ";
		delSql = delSql + " where doc_id = ? ";
		delSql = delSql + " and contract_id = ? ";
		delSql = delSql + " and  mod_reason = 'his_rate_change' "; 
		delSql = delSql + " and( mod_status = 'his_status_before' or mod_status = 'his_status_after' )";//前：his_status_before ，后：his_status_after
		log.info("调息撤销or流程草稿废弃时删除HIS表数据："+delSql);
		this.tableService.getBussinessDao().getJdbcTemplate().update(delSql,docId,contractId);
	}
	...
	private String getDelHisSql(String docid,String mod_status,String mod_reason){
		String delSql = " delete contract_fund_rent_plan_his ";
		delSql = delSql + " where doc_id = ? ";
		delSql = delSql + " and mod_status = ? "; 
		delSql = delSql + " and mod_reason = ? ";
	 ...
	 * @return
	 */
	private String getDelHisSql(String docid, String contractId,String mod_status,String mod_reason){
		String delSql = " delete contract_fund_rent_plan_his ";
