	...
	function loadWorkflowAttachment() {
		...
		if(!window.initFileListParams){
			apAttachmentFileUpload({
				...
				batchDownloadAttachmentFileTitle:window.assignBatchDownloadAttachmentFileTitle||"<mini:param name='proj_info.custname'/>",
				//attachmentFileDictListImplBeanName:'commonAttachmentFileDictList',
				attachmentType:window.assignAttachmentTypeIds||'<mini:param name="currentTaskActivityDetailInfo.attachmentTypeIds"/>',
				identifierOne:window.assignAttachmentKeyOne||"<mini:param name='currentProcessInstanceDBID'/>",
