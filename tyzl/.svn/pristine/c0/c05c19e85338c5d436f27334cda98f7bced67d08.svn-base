...
public class RentChargeServiceImpl implements RentChargeService {
	...
	public Hashtable<String ,Object> rentChargeCal(AdjustBean adb, String calType, Hashtable<String ,Object> ht, String[] rentAdjustList) throws Exception {
		...
		if (adb.getOnHireId() != null && !adb.getOnHireId().equals("")) {
			wheresql += " and onHire_id=?";
			paramValue.add(adb.getOnHireId());
			paramType.add(Types.VARCHAR);
		}
		//( 根据查取交易结构信息)
		...
		cdb = cddao.getConditionBeanByContract(whereMap, tcb);
		...
		try {
			...
			// 等额租金法
			if ("even_rent".equals(cdb.getSettleMethod()) || "irregular_rent".equals(cdb.getSettleMethod())) {
				// 为了让等额租金中覆盖这两个值所以要滞空
				...
				/*在com.tenwa.leasing.rentcharge.web.rentChargeCal方法if分支中进行单独处理的
				if (adb.getAdjustType().equals("his_contract_end")) {
					// 合同中途终止
					rpcs = new EvenCorpusPlanStopCalServiceImpl();
				}
				*/
