...
public class UploadBean {
	 ...
	 * @param objectPath
	 */
	public void setObjectPath(String objectPath) throws Exception{
		File file = new File(StrTools.fileNameCheck(objectPath));
		if (file.exists()) {
	 ...
	 *             抛出IO读取出错异常
	 */
	public void setSourceFile(HttpServletRequest request) throws Exception {
		...
		if (!successful) {
			for (int d = 0; d < count; d++) {
				File tmp = new File(StrTools.fileNameCheck(objectPath + objectFileName[d]));
				tmp.delete();
			}
		}
	}
	 ...
	 * @param i
	 */
	private void transferFile(int i) throws Exception {
		String x = userid + Long.toString((new Date()).getTime());
		FileOutputStream out = null;
		try {
			objectFileName[i] = x + "." + suffix[i];

			 out = new FileOutputStream(StrTools.fileNameCheck(objectPath + objectFileName[i]));
			int a = 0;
			...
			if (!successful) {
				sis.close();
				File tmp = new File(StrTools.fileNameCheck(objectPath + objectFileName[count]));
				tmp.delete();
			}
