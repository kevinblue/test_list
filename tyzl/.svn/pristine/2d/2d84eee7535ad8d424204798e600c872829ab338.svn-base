...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
			...
			data: JsonUtil.decode('<mini:param  name="json_proj_equip_str" defaultValue="[]"/>'),
			afterShowWindowCallBack:function(miniTable,miniForm,cflag){
			   //回租下，供应商和制造商非必填
			   var leasetyp=mini.get("id_leasform").getValue();
			   if(leasetyp=="lease_form2" || leasetyp=="lease_form4"){
				   getMiniEditFormField("proj_equip.vndr").setRequired(false);
				   getMiniEditFormField("proj_equip.manufacturer").setRequired(false);
			   }
			},
			columns: [
				{type:'indexcolumn'},
...
function adjustTotalPrice(e){
	var equipnum =getMiniEditFormField("proj_equip.equipnum").getInputText().trim();
	var price = getMiniEditFormField("proj_equip.price").getInputText().trim().replace(/,/g,"");
	equipnum = equipnum || 0;
	price = price || 0;
	var s=e.sender;
    if (!price.match(/^(:?(:?\d+.\d+)|(:?\d+))$/)){price=0;
    getMiniEditFormField("proj_equip.price").text=0;}else{
    	
    }
	var temp = Number(equipnum) * Number(price);
	if(isNaN(temp)){temp=0;}
	getMiniEditFormField("proj_equip.equipprice").setValue(formatNumberThousand(decimal(temp,2)));
...
function checkEquipIsNull(){
	...
	for ( var i = 0; i < jsondata.length; i++) {
		totalPrice += parseFloat(jsondata[i]['nowtotal']);
	}
	//计算商务条件的设备款
