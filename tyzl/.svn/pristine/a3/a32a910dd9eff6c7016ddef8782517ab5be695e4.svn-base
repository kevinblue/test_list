...
//	private VoucherToEasService voucherToEasService;
	...
	public IntereasVoucherEntries saveIntereasVoucherentries(IntereasVoucherEntries intereasVoucherentries,DictionaryData dictionaryData,IntereasVoucherHead intereasVoucherHead,User user) throws Exception {
        ...
        if (intereasVoucherentries == null || "".equals(intereasVoucherentries)) {  
        	...
        	Map<String,Object> map = this.getSql(intereasVoucherentries, dictionaryData, intereasVoucherHead);
        	...
        	String filed = map.get("filed").toString();
        	//凭证头日志字符串
	...
	public IntereasVoucherEntries saveIntereasVoucherentriesJdbc(IntereasVoucherEntries intereasVoucherentries,DictionaryData dictionaryData,IntereasVoucherHead intereasVoucherHead,User user) throws Exception {
		...
		if (intereasVoucherentries == null || "".equals(intereasVoucherentries)) {  
			...
			log.debug("凭证体SQL："+sql);
			//方式2：通过SQL方式更新
			this.getBussinessDao().getJdbcTemplate().update(sql,(Object [])map.get("sqlparam"),(int [])map.get("sqlparamtype"));
	 ...
	 * @return
	 */
	private Map<String,Object> getSql(IntereasVoucherEntries intereasVoucherentries,DictionaryData dictionaryData,IntereasVoucherHead intereasVoucherHead){
    	...
    	param.add(uuId);
    	param.add(voucherNumber);
    	param.add(entrySeq);
    	param.add(voucherAbstract);
    	param.add(accountNumber);
    	param.add(entryDC);
    	param.add(currencyNumber);
    	param.add(MoneyUtils.formatNumberDoubleTwo( MoneyUtils.getZeroStr( String.valueOf( originalAmount ) ) ));
    	param.add(MoneyUtils.formatNumberDoubleTwo( MoneyUtils.getZeroStr( String.valueOf( debitAmount ) ) ));
    	param.add(MoneyUtils.formatNumberDoubleTwo( MoneyUtils.getZeroStr( String.valueOf( creditAmount ) ) ));
    	int[] argtype=new int[10];
    	argtype[0]=Types.VARCHAR;
    	argtype[1]=Types.VARCHAR;
    	argtype[2]=Types.VARCHAR;
    	argtype[3]=Types.VARCHAR;
    	argtype[4]=Types.VARCHAR;
    	argtype[5]=Types.VARCHAR;
    	argtype[6]=Types.VARCHAR;
    	argtype[7]=Types.DOUBLE;
    	argtype[8]=Types.DOUBLE;
    	argtype[9]=Types.DOUBLE;
    	//封装
    	map.put("message", message);
    	map.put("sql", sql.toString());
    	map.put("filed",filed.toString());
    	map.put("sqlparam", param.toArray());
    	map.put("sqlparamtype", argtype);
		return map;
	}
...
//		  where user_constraints.constraint_name='FK45607DE660FE33E4'
	...
	public int getNumber(String voucherNumber) throws DataAccessException, Exception{
		String sql = "   select nvl(max(entrySeq),0)+1 as entrySeq from  intereas_voucherentries  where   voucherNumber = ? ";
		log.error("查询凭证体行号SQL:"+sql);
		int entrySeq = 1;
		entrySeq = this.getBussinessDao().getJdbcTemplate().queryForInt(sql,voucherNumber);
		return entrySeq;
	}
