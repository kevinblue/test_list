...
public class RentPlanContrCalDAOImpl {
	...
	public boolean addRentPlan(String tbName, FundRentPlanBean fpb, TabCalBean tcb, int startList) throws Exception {
		...
		if (fpb != null && fpb.getRentList() != null && fpb.getRentList().size() > 0) {
			...
			List<String> actualRentList = fpb.getAclRentList();// 租金列表
			if(null==actualRentList||actualRentList.size()==0){
				actualRentList=rent_list;
			}
			List<String> planDate_list = fpb.getPlanDateList();// 日期列表
			List<String> calPlanDate_list = fpb.getCalPlanDateList();// 日期列表
			if(null==calPlanDate_list||calPlanDate_list.size()==0){
				calPlanDate_list=planDate_list;
			}
			...
			for (int i = 0; i < rent_list.size(); i++) {
				...
				sql += "'" +Tools.getDBDateStr( calPlanDate_list.get(i) ) + "',";
				sql +=NumberUtils.nullToZero( actualRentList.get(i) ) + ",";
				...
				conn.executeUpdate(sql,new ArrayList(),new ArrayList(), "流程类型:" + tcb.getCalType() + "|=====|table:" + tbName + "添加租金计划语句...");
				sql = "";
			}
		}
	...
	public boolean deleteRentPlan(String tbName, TabCalBean tcb, int startList) throws Exception {
		Conn conn = new Conn();
		String sql = " delete " + tbName + " where " + tcb.getContOrProjCName() + "=? and doc_id=? and rent_list>=?";
		...
		conn.executeUpdate(sql,paramValue,paramType, "流程类型:" + tcb.getCalType() + "删除租金计划...");
	...
	public FundRentPlanBean getRentAndDateByTcb(TabCalBean tcb, int startList) throws Exception {
		...
		Conn conn = new Conn();
		String sql = " select * from " + tcb.getRentPlan_tb() + " where " + tcb.getContOrProjCName() + "=? "  ;
		List paramValue=new ArrayList();
		List paramType=new ArrayList();
		paramValue.add(tcb.getContOrProjCValue());
		paramType.add(Types.VARCHAR);
	 ...
	 * @throws Exception
	 */
	public boolean addRentInfoBySql(String toTb, String frTb, Map wherefrsql, Hashtable<String, String> colHt) throws Exception {
		// 数据库操作对象
		Conn conn = new Conn();
		...
		sql += ",quot_id";
		sql += ",onhire_id  ";
		sql += " FROM " + frTb + wherefrsql.get("where");
		conn.executeUpdate(sql,(List)wherefrsql.get("value"),(List)wherefrsql.get("type"), "添加租金计划 从表" + frTb + "到表" + toTb + " ");
		return true;
	}
	 ...
	 * @throws Exception
	 */
	public FundRentPlanBean getRentInfoForCash(String tbName, Map wheresql) throws Exception {
		...
		String sql = " select (case when rent is null then 0 else rent end) rent,plan_date from " + tbName + " where " + wheresql.get("where");
		Conn conn = new Conn();
		List<Map<String, String>> rs_list = conn.executeQuery(sql,(List)wheresql.get("value"),(List)wheresql.get("type"), "租金计划现金流所需数据查询....");
		for (Map<String, String> rs : rs_list) {
	...
	public boolean updateRentPlanDate(String tbName, TabCalBean tcb, int startList, List<String> dateList) throws Exception {
		...
		for (int i = 0; i < dateList.size(); i++) {
			List paramValue=new ArrayList();
			List paramType=new ArrayList();
			paramValue.add(dateList.get(i).toString());
			paramType.add(Types.VARCHAR);
			paramValue.add(tcb.getContOrProjCValue() );
			paramType.add(Types.VARCHAR);
			paramValue.add(tcb.getDocId() );
			...
			paramType.add(Types.INTEGER);
			String sql = "";
			sql += "  update " + tbName + " set plan_date=? where " + tcb.getContOrProjCName() + "=? and doc_id=? and rent_list=? "  ;
			// 报价编号
			...
			startList = startList + 1;
			conn.executeUpdate(sql,paramValue,paramType, tcb.getCalType(), "更新租金计划时间sql：");
		}
		// 如果是多次起租,那么合同表也要做同样的操作
	...
	public boolean insertRentPlanOnHire(TabCalBean tcb, String star_date) throws Exception {
		...
		// 插入
		addRentPlan(tcb.getRentPlan_tb(), frpb, tcb, 1);
		...
		where = " where " + tcb.getContOrProjCName() + "=? and doc_id=? ";
		// 报价编号
		if (!"".equals(tcb.getOnHire_id())) {
		...
		paramValue.add( tcb.getDocId());
		paramType.add(Types.VARCHAR); 
		List<Map<String, String>> rs_list = db.executeQuery(sql + where,paramValue,paramType);
		String column_1 = "";
		String rent_list = "";
		String end_date = "";
		String leas_term = "";
		...
		for (Map<String, String> rs : rs_list) {
			...
			List tempValue=new ArrayList();
			List tempType=new ArrayList();
			tempValue.addAll(paramValue);
			tempType.addAll(paramType);
			tempValue.add(rent_list);
			tempType.add(Types.INTEGER);
			List<Map<String, String>> rs_inner_list = db.executeQuery(sql + where + " and rent_list<?",tempValue, tempType);
			for (Map<String, String> s_inner : rs_inner_list) {
				...
				tempValue.add(rent_list);
				tempType.add(Types.INTEGER);
				db.executeUpdate(sql,tempValue,tempType);
		...
		db.executeUpdate(sql,paramValue,paramType, "多次起租中在获得了汇总租金计划之后更新交易结构表");
		return true;
	}
	...
	public boolean insertFinaRentPlanOnHire(TabCalBean tcb, String star_date) throws Exception {
		...
		frpb.setRentAdjustList(null);// 汇总不记录调整列
		// 插入
		addRentPlan(tcb.getRentFinaPlan_tb(), frpb, tcb, 1);
		// 因为在均息下本金在合并后有差异.所以要做最后一期的调整
		String sql = "";
		String where = "";
		List paramValue=new ArrayList();
		...
		for (Map<String, String> rs : rs_list) {
			...
			paramValue.add(rent_list);
			paramType.add(Types.INTEGER);
			List<Map<String, String>> rs_inner_list = db.executeQuery(sql + where + " and rent_list<?",paramValue,paramType );
			for (Map<String, String> rs_inner : rs_inner_list) {
				String corpus = rs_inner.get("corpus");
				sql = "update " + tcb.getRentFinaPlan_tb() + " set corpus=" + column_1 + "-" + corpus + where + " and rent_list=?";
				db.executeUpdate(sql,paramValue,paramType);
				sql = "update " + tcb.getRentFinaPlan_tb() + " set interest=rent-corpus " + where + " and rent_list=?";
				db.executeUpdate(sql,paramValue,paramType);
			}
		}
	...
	public void updateEndPlanDate(TabCalBean tcb) {
		String sql = "select start_date,income_number_year,lease_term from " + tcb.getCondition_tb() + " where " + tcb.getContOrProjCName() + "=? and doc_id=? ";
		List paramValue=new ArrayList();
		List paramType=new ArrayList();
		paramValue.add(tcb.getContOrProjCValue());
		paramType.add(Types.VARCHAR);
		...
		String star_date = "";
		try {
			List<Map<String, String>> rs_list = db.executeQuery(sql,paramValue,paramType);
			...
			for (Map<String, String> rs : rs_list) {
				...
				sql = "select max(rent_list)rent_list from " + tcb.getRentPlan_tb() + " where " + tcb.getContOrProjCName() + "=? and doc_id=? ";
				List<Map<String, String>> rs_inner_list = db.executeQuery(sql,paramValue,paramType);
				for (Map<String, String> rs_inner : rs_inner_list) {
					sql = "update " + tcb.getRentPlan_tb() + " set plan_date='" + star_date + "' where " + tcb.getContOrProjCName() + "=? and doc_id=? and rent_list=?" ;
					paramValue.add(rs_inner.get("rent_list"));
					paramType.add(Types.INTEGER);
					db.executeUpdate(sql,paramValue,paramType, "变更或者调息 根据起租日更新最后一期租金支付日");
				}
			}
		} catch (Exception e) {
	...
	public void updateDataByRentList(FundRentPlanBean fpb, AdjustBean adb, TabCalBean tcb) throws Exception {
		...
		sql.append(" update " + tcb.getRentPlan_tb() + " set  ")
		   ...
		   .append(" where " + tcb.getContOrProjCName() + "=? " )
		   .append("");
		if (tcb.getDocId() != null && !tcb.getDocId().equals("")) {
		...
		paramValue.add( adb.getStartList());
		paramType.add(Types.INTEGER);
		Conn db = new Conn();
		
		db.executeUpdate(sql.toString(),paramValue,paramType, "合同中途终止修改最后一期利息");
		db.close();
	}
	...
	public void updateDataByDiscussrent(AdjustBean adb, TabCalBean tcb) throws Exception {
		...
		Conn db = new Conn();
		List<Map<String, String>> list = db.executeQuery(finalSql.toString(),paramValue,paramType, "查询小于等于约定终止日的最大偿还期项加1即为最后的终止期项");
		String rent_list = "0";
		if(list.size() > 0){
		...
		paramValue.add(tcb.getDocId());
		paramType.add(Types.VARCHAR);
		db.executeUpdate(sql.toString(),paramValue,paramType, "合同中途终止删除大于约定终止日的所有偿还计划");
	    ...
	    paramType=new ArrayList();
		//7.执行合同终止最终一期持久化临时表操作
		db.executeUpdate(sql.toString(),paramValue,paramType, "合同中途终止插入最终的终止测算利息");
		
		db.close();
	}
	
	public int findNoIncomeRentPlanMinList(ConditionBean cb) throws Exception {
		Conn conn = new Conn();
		String sql = "select min(a.rent_list) rentlist from (select cfrp.rent_list,nvl(cfri.rent,0) rent,nvl(cfri.rent_adjust,0) rent_adjust from contract_fund_rent_plan cfrp left join (select sum(cfri.rent) rent ,sum(cfri.rent_adjust) rent_adjust,cfri.plan_id from contract_fund_rent_income cfri where cfri.roll_back is null group by cfri.plan_id ) cfri on cfrp.id=cfri.plan_id  where cfrp.contract_id=(select id from contract_info where contract_id = '"+cb.getContractId()+"')) a where a.rent=0 and a.rent_adjust=0"; 
		List paramValue=new ArrayList();
		List paramType=new ArrayList();
		paramValue.add(cb.getContractId());
		paramType.add(Types.VARCHAR);
		List<Map<String, String>> rs_list = conn.executeQuery(sql, paramValue,paramType,"查询未回笼最小期次");
		int startlist=0;
		for (Map<String, String> rs : rs_list) {
			if(null!=rs.get("rentlist")&&!"".equals(rs.get("rentlist"))){
				startlist=Integer.parseInt(rs.get("rentlist"));
			}
		}
		return startlist;
	}
	public int findNoIncomeRentPlanMinListChange(ConditionBean cb) throws Exception {
		Conn conn = new Conn();
		String sql = "select min(a.rent_list) rentlist from (select cfrp.rent_list,nvl(cfri.rent,0) rent,nvl(cfri.rent_adjust,0) rent_adjust from contract_fund_rent_plan cfrp left join (select sum(cfri.rent) rent ,sum(cfri.rent_adjust) rent_adjust,cfri.plan_id from contract_fund_rent_income cfri where cfri.roll_back is null group by cfri.plan_id ) cfri on cfrp.id=cfri.plan_id  where cfrp.contract_id='"+cb.getContractId()+"') a where a.rent=0 and a.rent_adjust=0"; 
		List paramValue=new ArrayList();
		List paramType=new ArrayList();
		paramValue.add(cb.getContractId());
		paramType.add(Types.VARCHAR);
