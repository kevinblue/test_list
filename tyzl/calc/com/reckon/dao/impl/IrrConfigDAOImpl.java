...
public class IrrConfigDAOImpl {
	...
	public List<CashConfigBean> getConfigInfo(String occu_type) throws Exception {
		...
		sql += " ,occu_type FROM fund_cash_config where occu_type like '%?%'";
		List paramValue=new ArrayList();
	    List paramType=new ArrayList();
	    paramValue.add(occu_type);
	    paramType.add(Types.VARCHAR);
		List<CashConfigBean> ccfb_list = new ArrayList<CashConfigBean>();
		List<Map<String, String>> rs_list = conn.executeQuery(sql,paramValue,paramType, "租金测算的现金流配置");
