...
public class GrossProfitDAOImpl {
	...
	public List<GrossProfitConfig> findGrossProfitConfig(int term) {
		...
		try {
			...
			String sql = "SELECT " + crossProfitConfigColumns + " FROM BASE_COARSE WHERE ? BETWEEN BEGIN_MONTH AND END_MONTH ORDER BY START_DATE_ ASC";
	...
	public GrossProfitConfig findLastGrossProfitConfig(int term, String startDate) {
		try {
			conn = (conn == null ? new Conn() : conn);
			String sql = "SELECT " + crossProfitConfigColumns + " FROM BASE_COARSE WHERE BEGIN_MONTH<=? AND END_MONTH>=? AND START_DATE_ <=? ORDER BY START_DATE_ DESC";
			logger.info("查询粗利配置信息：" + sql);
			List paramValue=new ArrayList();
		    List paramType=new ArrayList();
		    paramValue.add(term);
