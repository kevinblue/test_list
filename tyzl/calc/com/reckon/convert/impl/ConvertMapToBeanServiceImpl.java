...
public class ConvertMapToBeanServiceImpl extends AbstractBaseServiceImpl implements ConvertMapToBeanService {
    ...
    public ConditionBean convertContionBean(ContractConditionTemp condition)  throws Exception {
    	...
    	conditionBean.setHandlingChargeRatio(condition.getHandlingChargeMoneyRatio()==null?"0":condition.getHandlingChargeMoneyRatio().toString());
...
//	}
	...
	public ConditionBean convertContionBeanByOriginal(ConditionBean conditionBean,Map<String, String> modelData)  throws Exception {
		...
		String endDate = Tools.getDBDateStr( modelData.get("enddate") );//结束日期
		//*********************************************************************************【测算条件结束！】
		
		//*********************************************************************************【资金信息开始：】
		String assetsratio=modelData.get("assetsratio");
		String assetsmoney=modelData.get("assetsmoney");
		String adviserratio=modelData.get("adviserratio");
		String advisermoney=modelData.get("advisermoney");
		...
		conditionBean.setHandlingChargeRatio(handlingChargeRatio);
		...
		conditionBean.setAdvisermoney(advisermoney);
		conditionBean.setAdviserratio(adviserratio);
		conditionBean.setAssetsmoney(assetsmoney);
		conditionBean.setAssetsratio(assetsratio);
