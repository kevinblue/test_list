...
public class TransRateDo {
	 ...
	 * @throws Exception
	 */
	public Hashtable<String, String> processTransRate(String adjustId, String contractInfoId, String contractId, String creator, String docId,List<SpecialRulesBean> srb) throws Exception {
		...
		cb.setContractId(contractInfoId);
		cb.setDocId(docId);
		cb.setProjId(contractInfoId);
		...
		// 查询租金计划
		FundRentPlanBean oldRentPlanContext = fundPlanDAO.findContractFundRentPlanList(contractInfoId);
		oldRentPlanContext.setBusinessId(contractInfoId);// CONTRACT_INFO.ID
		oldRentPlanContext.setContractId(contractId);// 投放编号
		...
		// 根据调息方式得到合同重新计算利息的起始日期
		String newDate = TransRateHelper.getNewDateByAdjustType(cb.getAdjustStyle(), PBOCInfo.get("start_date").toString());
		//计算调息开始期项
...
//		}
		...
		} else if ("fixed".equals(cb.getRateFloatType())) {
			msgs += "\\n合同号:" + contractId + "不满足调息规则!\\n合同商务条件为固定不调息合同.\\n未进行调息.";
		...
		//注释
//		else if(cb.getStartTransRateLine() != null && cb.getStartTransRateLine().length() > 0){
//			String newBaseRate = TransRateHelper.getBaseRateApplyCondition(cb, PBOCInfo);
//			if(new BigDecimal(newBaseRate).compareTo(new BigDecimal(cb.getStartTransRateLine())) < 0){
//				msgs += "\\n合同号:" + contractId + "不满足调息规则!\\n央行基准利率小于设定的基本调息利率.\\n未进行调息.";
//			}
//		}
		...
		if (msgs.length() > 0) {
			...
			String oldYearRate = oldRentPlanContext.getYearRateList().get(startList - 1);
			logger.info("调息旧年利率...." + oldYearRate);
			String newYearRate = TransRateHelper.getNewRate(cb, oldYearRate, PBOCInfo);
			logger.info("调息新年利率...." + newYearRate);

			// 清空原来的临时流程数据
			fundPlanDAO.cleanContractConditionTemp(docId, contractInfoId);
			fundPlanDAO.cleanContractRentPlanTemp(docId, contractInfoId);
			fundPlanDAO.cleanContractCashDetailTemp(docId, contractInfoId);

			// 调息上下文信息
			InterContBean icb = new InterContBean();
			icb.setContractId(contractInfoId);
			...
			// 调息
			processPmtTranRate(cb, oldRentPlanContext, icb);
			
			newYearRate = String.valueOf( icb.getNewYearRate() );
			
			// 财务计划调息
			if (!(cb.getSettleMethod().equals("even_rent") && "next_list".equals(cb.getAdjustStyle()))) {// 等额租金时财务计划不需再处理,因为上面调息时处理业务租金计划时已经同时设置了财务租金计划
				TransRateHelper.processFinacRentPlan(cb, oldRentPlanContext, icb);
			}

			// 保存临时租金计划到临时表
			oldRentPlanContext.setContractId(contractInfoId);
			...
			tcb.setContOrProjCValue(contractInfoId);
			...
			queryParams.put("contractId",  contractInfoId);
			
			rentPlanList = tableService.findEntityByProperties(ContractFundRentPlanTemp.class, queryParams);
			
			hsql = " FROM  ContractCashDetailTemp where contractId = '"+contractInfoId+"' AND docId = '"+cb.getDocId()+"' order by onhireId ";
			...
			// 计算现金流实际IRR
			String realIrr = IrrTools.getIrr(cashDetailList, process);
			icb.setNewIrr(realIrr);
			
			RentCalculateServiceImpl rcs = new RentCalculateServiceImpl();
			BigDecimal grossProfit = rcs.getGrossProfitR(oldRentPlanContext.getCorpusFinacList(), cb, process);
	...
	private static void processPmtTranRate(ConditionBean cb, FundRentPlanBean oldRentPlanContext, InterContBean icb) throws Exception {
		...
		rtrs.processPmtTranRate(cb, oldRentPlanContext, icb);
	}
}
