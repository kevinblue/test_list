...
public class RentalServiceImpl {
	...
	public String getSignRentByCb(ConditionBean cb) {
		...
		String preRate = RateTools.getPreRate(cb.getYearRate(), cb.getIncomeNumberYear());
	...
	public List<String> getDeviRentList(ConditionBean cb, String[] rentAdjustList, List<String> year_rate) {
		...
		// 总的
		String preRate = RateTools.getPreRate(year_rate.get(year_rate.size() - 1), cb.getIncomeNumberYear());
	...
	public List<String> getRentValue( List<KnowingRentBean> kniwnRentBeans) {
		...
		for (KnowingRentBean obj : kniwnRentBeans) {
			...
			String rent = MoneyUtils.getZeroStr( obj.getRent() );//对应租金值  
...
public List<String> getRentListByCondForSpecial(ConditionBean cb,SpecialRulesBean specialRulesBean,int...str) {
		...
		String preRate = new BigDecimal(cb.getYearRate()).divide(new BigDecimal(100),Scale.RATE_SCALE,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(specialRulesBean.getRegular_months())).divide(new BigDecimal(12),Scale.RATE_SCALE,BigDecimal.ROUND_HALF_UP).toString();
		logger.debug("PMT期利率:"+preRate);
		String rent = getSignRentByCbForSpecial(cb,rateReCal,preRate,specialRulesBean.getRegular_months());
		String preRateold = "";
		String rentold = "";
		...
		for(int i=startlist;i<=specialRulesBean.getEndList();i++){
			rent_list.add(rent);
		}
		return rent_list;
	}
