...
//	}
	...
	public String toV8(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(res != null){
			...
			try {
				...
				*/
				//方式2
				outtemp=new FileOutputStream(StrTools.fileNameCheck(filename), true);
				osw = new OutputStreamWriter(outtemp, "GB2312");
				osw.write(res);
				osw.flush();
		       ...
		       //***********************************************************************************
				...
				throw ioexception;
				    ...
				    if(null!=outtemp)FileUtil.safeCloseOutStream(outtemp);
					if(null!=bis)FileUtil.safeCloseInputStream(bis);   
					if(null!=bos)FileUtil.safeCloseOutStream(bos);
					if(null!= osw )FileUtil.safeCloseOutputStreamWriter(osw);
	...
	private void copyFile(File oldPath, String newPath) { 
       ...
       try { 
    	   ...
    	   if(null!=inStream){FileUtil.safeCloseInputStream(inStream);}
	...
	public String cXML(String sPath,String data){
	        ...
	        OutputStreamWriter osw=null;
	        try {
	         File f = new File(StrTools.fileNameCheck(sPath));
	         if (f.exists()) {
	          ...
	          ff = new FileWriter(f);
	         java.io.FileOutputStream writerStream = new java.io.FileOutputStream(StrTools.fileNameCheck(sPath));    
	          osw=new java.io.OutputStreamWriter(writerStream, "GB2312");
	         ...
	         return null;
	        	    ...
	        	    if(null!=osw){FileUtil.safeCloseOutputStreamWriter(osw);}
					if(null!=utput){FileUtil.safeCloseBufferedWriter(utput);}
					if(null!=ff){FileUtil.safeCloseOutputStreamWriter(ff);}
	        }
	 }
