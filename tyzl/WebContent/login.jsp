...
<head>
	...
	<script type="text/javascript">
		...
		function MM_preloadImages() { //v3.0
			...
			if('true' == '<mini:param name="login_password_error"/>') {
				userPassword.focus();
			} else {
				userInput.focus();
			}
		...
		jQuery(function (){
			var message="";
			if('<mini:param name="login_username_error"/>' == 'true'){
				message="用户名";
			}
			if('<mini:param name="login_password_error"/>' == 'true'){
			...
			if('<mini:param name="login_verification_error"/>' == 'true'){
				alert("请你输入正确的验证码！");
			}
			if('<mini:param name="login_usermac_error"/>' == 'true'){
				alert("请用自己的计算机登陆此账号！");
			}
			...
			<div class="loginbox" id='id_loginBox'>
				<form name="login_form_" id="login_form_" action="<c:url value='j_spring_security_check'/>" method="post" >
					<div style="padding-left: 280px; padding-top: 70px;">
						...
						<div style="margin-top: 20px;">
							<input type="password" class="loginpwd" id="j_userkey" name="j_userkey">
						</div>
