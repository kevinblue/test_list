	 ...
	 <script type="text/javascript" >
		<!--
		 var loadInfo = function(){
			if('empty' == '<mini:param name="checkUniqueComputerPass"/>')
			{
				alert("未检测到该机器的硬盘号，请检查是否已安装所需浏览器插件，或者没有加入受信站点！");
		    }
			if('false' == '<mini:param name="checkUniqueComputerPass"/>')
			{
				alert("请使用已注册的电脑登录（<mini:param name="checkUserName"/>）");
		    }
			if('true' == '<mini:param name="login_username_error"/>')
			{
