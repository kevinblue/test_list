...
<html>
	<head>
		...
		<script type="text/javascript" >
			...
			if(confirm("用户<mini:param name="isHasExistUserRealName"/>（<mini:param name="isHasExistUserName"/>）已在线，是否继续登录，原用户将被强行注销？")){
				ajaxRequest({
		...
		<div class="header" style="overflow:hidden;" >
			...
			<div class="main_topbg" id='id_mainPage_header'>
				...
				<div class="topright">	
					...
					<a href="javascript:void(0);" onClick="javascript:shwoMessageWindwos(true);" ><b id="emailNum"><mini:param name="noreadmsg_count"/></b></a>
					</div>
				</div>
					...
					if(data.responseText){
						...
						for (var i = 0; i < len; i++) {
							...
							if(nowTime.getTime() - taskTime.getTime() <= 310000){// 310秒内的信息进行提醒，300秒检查一次
								...
								jQuery('<a href="javasript:void(0);" onclick="viewCurrentProcessForm('+ task.taskid +',\'' + task.actorid + '\',\'' + task.tasktype + '\');">' + msg.join(" -> ") + '</a>').appendTo(divBox);
								showBeNotice = true;
							}
						}
	...
	<script type="text/javascript" defer>
		...
		jQuery(function(){
			...
			var mynotice = new tracywindyTable({
				...
				params:{
					MSG_TYPE:'msgtype.2',
					USER_ID:'<mini:user valuetype="id" />'
				}
			});
		...
		function markAsRead(readstatus,isprompt){
			...
			var params = {message_bid:messageBid,user_id:"<mini:param name='login_userid'/>",read_status:readstatus};
			ajaxRequest({
				method:'POST',
				url:url,
				params:params,
