					 	...
					 	<span class="print-tabs-title-content">附件一览</span>
					 	    <c:forEach var="projSummaryHistoryInfo" items="${projSummaryHistoryInfos}">
								...
								<div class="toggle-icon-expanded" title="折叠"
									...
									<c:set var="tworkflowname" value="${projSummaryHistoryInfo.workflowName}"></c:set>
									<mini:param  name="tworkflowname" />
								</div>
								<div class="workflow-attachment-${projSummaryHistoryInfo.historyProcessInstanceImpl.dbid}" style="line-height:20px;">
							             <c:forEach var="attachmentDetailInfo" items="${projSummaryHistoryInfo.historyProcessInstanceImpl.attachmentFileUploadInfoDetails}">
							                 <c:set var="t_attachmentDetailInfoid" value="${attachmentDetailInfo.id}"></c:set>
							                 <c:set var="t_chineseFileName" value="${attachmentDetailInfo.chineseFileName}"></c:set>
							                 <c:set var="t_fileType" value="${attachmentDetailInfo.fileType}"></c:set>
							                 <c:set var="t_chineseFileName" value="${attachmentDetailInfo.chineseFileName}"></c:set>
							                 <c:set var="t_uploadTime" value="${attachmentDetailInfo.uploadTime}"></c:set>
							                 <c:set var="t_realname" value="${attachmentDetailInfo.uploadUser.realname}"></c:set>
							                 
							                 <div>&nbsp;<a><img align='absmiddle' width=14  src='${pageContext.request.contextPath}/images/attach.gif'/></a><a href="javascript:void(0);" onclick="downloadUploadFile('<mini:param  name="t_attachmentDetailInfoid" />')">								                
									                 <mini:param  name="t_chineseFileName" />【<mini:param  name="t_fileType" />】
									                 <c:choose> 
