...
var isDraft = ("Draft" == "<mini:param  name='processInstanceState'/>");

var __getDocId = function(){// JBPM流程ID
	return '<mini:param  name="currentProcessInstanceDBID"/>';
};
var __getProcessId = function(){// 流程实例ID
	return '<mini:param  name="currentProcessInstanceId"/>';
};
var __getProcessName = function(){// 流程名称
	return '<mini:param  name="currentWorkFlowDisplayName"/>';
};
var __getProcessState = function(){// 流程状态
	return '<mini:param  name="processInstanceState"/>';
};
var __getCurrentTaskId = function(){// 当前任务ID
	return '<mini:param  name="currentTaskId"/>';
};
var __getCurrentTaskName = function(){// 当前任务名称
	return '<mini:param  name="currentTaskName"/>';
};
var __getCurrentUserId = function(){// 当前登陆用户ID
	return '<mini:param  name="CurrentUserId"/>';
};
var __getCurrentUserName = function(){// 当前登陆用户名称
	return '<mini:param  name="CurrentUserName"/>';
};
</script>
<script type="text/javascript">
	//显示流程图
	function toProcessActivePicture(){
		...
		var attachmentParams = {
			...
			processInstanceId:escape(encodeURIComponent('<mini:param  name="currentProcessInstanceId"/>')),
	...
	function viewProjSummary(){
		<c:if test="${empty currentHistoryTaskInfo.keyOne}">
		    mini.alert("流程关键字为空,请先设定流程《<mini:param  name='currentHistoryTaskInfo.workflowName'/>》的第一个流程关键字");
		    return;
		</c:if>
	    var URL = "${pageContext.request.contextPath}/jbpm/getProjSummaryHistoryInfos.action?keyOne=<mini:param  name='currentHistoryTaskInfo.keyOne'/>&proj_id=<mini:param name='project_info.projId'/>";
	...
	function previewPrint(){
	    var currentTaskId = "<mini:param name='currentTaskId'/>";
	    var URL = "${pageContext.request.contextPath}/jbpm/preivewProcessTaskForm.action?currentTaskId="+currentTaskId+"&jbpmWorkflowHistoryInfoUserId=<mini:param name='jbpmWorkflowHistoryInfoUserId'/>";
	...
	function flowDataContrast(){
		...
		if(flag=="compare"){
			var keyone="<mini:param  name='currentHistoryTaskInfo.keyOne'/>";
