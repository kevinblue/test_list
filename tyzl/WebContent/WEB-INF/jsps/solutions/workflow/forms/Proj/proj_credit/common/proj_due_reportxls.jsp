						...
						handler : function(miniTable, buttonText) {
							var params={};
						     ...
						     for(var i=0;i<cdates.length;i++){
				                    if(cdates[i].filename=="客户尽职调查报告.docx"){
				                   	 mini.alert("您已经生成客户尽职调查报告！");
				                         //return false;
				                    }
							    }	
							var fileTeplate=new fileTemplateConfig({
								templateno : 'F-201412163',
			...
			operValidate:function(miniTable, rowDatas,flag){
				  if(flag=="remove"){
				   for(j=0;j<rowDatas.length;j++){
					   if(rowDatas[j].filename!="客户尽职调查报告.docx"){
	                  	 mini.alert("您只能删除客户尽职调查报告！");
	                        return false;
	                   }
				   }}
			       return true;
...
function showOperator(temp,cfalg){
	...
	if(filename=="客户尽职调查报告.docx"&&cfalg){
	    return base2+"&nbsp;&nbsp;|&nbsp;&nbsp;"+base3;
	}else{
		return base2+"&nbsp;&nbsp;|&nbsp;&nbsp;"+base3;
	}
