...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
			...
			multiSelect: true,
			title:'本次调息',
			xmlFileName:'/eleasing/workflow/rent/regulating_breathing/current_regulating_breathing_list.xml',
			params:{ adjustid:'<mini:param  name="fund_standard_interest.id"/>',docid:flowUnid,Central_Bank_id:'<mini:param  name="Central_Bank_id" defaultValue="[]"/>'},
			tools : [
							{
								html : '撤销',
								plain : true,
		     ...
		     columns:[
						...
						{header:'投放编号',field:'contractputnumber'},
						{header:'客户名',field:'custname'},						
						{header:'利率浮动类型',field:'ratefloattypename'},
						{header:'调息时机',field:'adjuststylename'},					
					    ...
					    {header:'操作',align:'center',width:120,
									renderer:function(e){
									var  table = e.record;
					                return "<a href='javascript:void(0);' onclick='previewhis("+JsonUtil.encode(table)+")'>对比</a>";}}
...
function previewhis(data){
   ...
   var params = {cid:data.id,contractputnumber:data.contractputnumber,docid:data.docid,read_only :false};
...
function revocationTX(rowDatas){
	if(confirm("确认对选中合同进行撤销?")){
		...
		for(var i=0;i<rowDatas.length&&isContinue;i++){
			showMsg(rowDatas[i]["contractputnumber"]+"开始请求撤销调息!");
			doSendTXCX(rowDatas[i]["id"],rowDatas[i]["contractputnumber"]);//调用调息ajax
		}
	}
}
function doSendTXCX(cid,contractNumber){
	ajaxRequest({
	     ...
	     params:{adjustid:"<mini:param  name="fund_standard_interest.id"/>",docid:flowUnid,contractid:cid},
	     timeout:30*1000,
	     async:true,//改为同步请求
