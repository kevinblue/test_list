...
<input name="nowRentList" id="nowPlanDate" type="hidden" value="<mini:param  name="nowListPlanDate"/>"  /><!-- 选择期次对应的还款时间 -->
<input name="lastRentList" id="lastPlanDate" type="hidden" value="<mini:param  name="lastListPlanDate"/>" /><!-- 选择期次对应的上一期的还款时间 -->
<div  title="变更条件"  id="changeInfoForm"> 
    <table class="fillTable" cellspacing="0" cellpadding="0">
			<tr class="tr-project-info tr-odd">
		        <td class="td-content-title" width="12%">变更开始期次:</td>
		        <td class="td-content" width="38%"> <input  class="mini-combobox" id="startlist" label="变更开始期次" name="fund_rent_adjust.startlist" onvaluechanged="adjustListchange" textField="text" valueField="value" vtype="int" required="true"  value="<mini:param  name="fund_rent_adjust.startlist"/>"   /></td>
		        <td class="td-content-title" width="12%">变更后总期次:</td>
		        <td class="td-content" width="38%"><input name="fund_rent_adjust.adjustlist" id="adjustlist" vtype="int" label="变更后总期次" required="true"  class="mini-textbox"  required="true"  value="<mini:param  name="fund_rent_adjust.adjustlist"/>" /></td>
		    </tr>
			<tr class="tr-project-info tr-even">
		        <td class="td-content-title" width="12%">付款时间:</td>
		        <td class="td-content" width="38%"> <input name="fund_rent_adjust.paydayadjust" id="paydayadjust" label="付款时间" required="true"  onvaluechanged="paydayAdustchange" class="mini-datepicker"  value="<mini:param  name="fund_rent_adjust.paydayadjust"/>"   required="true" /> </td>
		        <td class="td-content-title"  width="12%">变更年利率:</td>
		        <td class="td-content" width="38%"><input  class="mini-textbox" vtype="double" required="true" label="变更年利率" name="fund_rent_adjust.yearrate" id="yearrate"  value="<mini:param  name="fund_rent_adjust.yearrate"/>"  /> </td>
		    </tr>
			...
			$.ajax({
		        ...
		        success: function (text) {
		        	...
		        	if(result.rentcalculaters == 'yes'){
			        	...
			        	mini.get("framework_condition.incomenumbershow").setValue(mini.get('adjustlist').getValue());
			        	...
			        	mini.get("framework_condition.leasetermshow").setValue(Number(mini.get('adjustlist').getValue())*Number(incomenumberyear));
		...
		function cancelRentChange(){
			...
			$.ajax({
		        ...
		        success: function (text) {
		        	...
		        	mini.get("framework_condition.irrshow").setValue(mini.get('irr').getValue());
		        	document.getElementById('framework_condition.irr').value = mini.get('irr').getValue();
		        	mini.get("framework_condition.incomenumbershow").setValue(mini.get('incomenumber').getValue());
		        	document.getElementById('framework_condition.incomenumber').value = mini.get('incomenumber').getValue();
		        	mini.get("framework_condition.leasetermshow").setValue(mini.get('leaseterm').getValue());
