...
public abstract class JedisManager {
  ...
  public byte[] getValueByKey(byte[] byteKey ){
		...
		try {
			...
			LoggerUtil.error(JedisManager.class, "获取byteKey为" + String.valueOf(byteKey)
					+ "的value失败", e);
		} finally {
  ...
  public void saveValueByKey(byte[] byteKey,byte[] byteValue){
		...
		try {
			...
			LoggerUtil.error(JedisManager.class, "保存byteKey为" + String.valueOf(byteKey)
					+ "保存byteValue为："+String.valueOf(byteValue), e);
		} finally {
			this.returnResource(jedis);
		}
  }
	  public void deleteByKeysPattern(byte[] keysPattern ){
			...
			try {
				...
				LoggerUtil.error(JedisManager.class, "删除byteKey为" + String.valueOf(keysPattern)
						+ "失败", e);
			} finally {
				this.returnResource(jedis);
			}
	...
	public void deleteByKey(byte[] byteKey){
		...
		try {
			...
			LoggerUtil.error(JedisManager.class, "删除byteKey为" + String.valueOf(byteKey)
					+ "失败", e);
		} finally {
			this.returnResource(jedis);
		}
	...
	public Set<byte[]> getKeysByKeysPattern(byte[] keysPattern){
		...
		try {
			...
			LoggerUtil.error(JedisManager.class, "获取byteKey为" + String.valueOf(keysPattern)
					+ "失败", e);
		} finally {
			this.returnResource(jedis);
		}
