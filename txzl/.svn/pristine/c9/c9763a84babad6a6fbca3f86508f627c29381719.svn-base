...
import com.reckon.util.EqDiffRentCalc;
import com.reckon.util.RentCaleCommonTools;
...
public class EqualDiffRentUtil {
	...
	public static RentInfoBox getRentInfoBox(ConditionBean conditionBean) {
		...
		String assets_value = conditionBean.getEquipEndValue();// 资产余值  【期末残值】
...
//		String Other_expenditure = conditionBean.getNominalprice();// 残值收入
		...
		calc.setIncome_number(String.valueOf( conditionBean.getIncomeNumber() ));// 期数
		calc.setLease_money(conditionBean.getCleanLeaseMoney());//
		calc.setFuture_money(conditionBean.getNominalPrice());// 留购价 
		// 起租类型  注意: 期初 数字 1 字符串 period_type_1  #分割线#  期末 数字0 字符串 period_type_0
		calc.setPeriod_type(conditionBean.getPeriodType());//  
		...
		String lease_term =  String.valueOf( conditionBean.getLeaseTerm() );
