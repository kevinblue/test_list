...
<input id="rawValue_contract_invoice_type.receivedinvoicetype" name="rawValue_contract_invoice_type.receivedinvoicetype" value="${requestScope['rawValue_contract_invoice_type.receivedinvoicetype']}" class="mini-textbox"  style="display:none">
<input id="rawValue_contract_invoice_type.rentinvoicetype" name="rawValue_contract_invoice_type.rentinvoicetype" value="${requestScope['rawValue_contract_invoice_type.rentinvoicetype']}" class="mini-textbox" style="display:none">
<input id="rawValue_contract_invoice_type.invoicetype" name="rawValue_contract_invoice_type.invoicetype" value="${requestScope['rawValue_contract_invoice_type.invoicetype']}" class="mini-textbox" style="display:none"/>
...
<div id="contract_rent_invoice_type_form" title="租金开票类型">
	<table cellspacing="0" cellpadding="0" style="width:100%;border: 1px solid #99CCFF;" class="fillTable">
			...
			<td class="td-content">
				<input id="contract_invoice_type.rentinvoicetype" name="contract_invoice_type.rentinvoicetype" class="mini-combobox" label="租金发票类型"
					textField="name"
					valueField="value"
					dataField="datas"
					allowInput="false"
		...
		<tr class="tr-even">
			...
			<td class="td-content">
				<input id="contract_invoice_type.taxregtype" name="contract_invoice_type.taxregtype" class="mini-combobox" textField="name"  label="纳税人类别" readOnly
		                  	   valueField="value"  
							   dataField="datas"
							   allowInput="false" 
			...
			<td class="td-content"><input id="contract_invoice_type.taxregcode" name="contract_invoice_type.taxregcode" class="mini-textbox" label="纳税人识别号" value="${requestScope['contract_invoice_type.taxregcode'] }" allowInput="true" /></td>
		</tr>
		<tr class="tr-odd">
...
//控制必填 一般纳税人必填
if("tax_level_name.1"=="${requestScope['contract_invoice_type.taxregtype'] }"){
	miniui_ext.setFieldsRequired("contract_rent_invoice_type_form",true);
}else{
	miniui_ext.setFieldsRequired("contract_rent_invoice_type_form",false);
	miniui_ext.setFieldsRequired("contract_invoice_type.receivedinvoicetype",true);
}
/* 
...
function receivedInvoie(e){
	var isYBNSR = mini.get("contract_invoice_type.taxregtype").getValue()=="tax_level_name.1"; //是否一般纳税人
...
<%-- function assignment(invoicetype,rentinvoicetype){
	$.ajax({
		...
		async:true,
		data :{
			xmlFileName:"/eleasing/workflow/fund/fund_put/query_rent_invoice_info.xml",
		...
		success: function (text) {
			...
			for(var i=0;i<result.length;i++){
				if(result[i].pid == 'rent_invoice_type') {
					mini.get("contract_invoice_type.rentinvoicetype").setValue(result[i].id);
					mini.get("contract_invoice_type.rentinvoicetype").setText(result[i].name);
					mini.get("rawValue_contract_invoice_type.rentinvoicetype").setValue(result[i].name);
					console.info(1+result[i].id  + result[i].name);
				}
				if(result[i].pid == 'invoicetype') {
					mini.get("contract_invoice_type.invoicetype").setValue(result[i].id);
					mini.get("contract_invoice_type.invoicetype").setText(result[i].name);
					mini.get("rawValue_contract_invoice_type.invoicetype").setValue(result[i].name);
