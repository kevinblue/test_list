...
public class FundPaymentStatusUpdateController {
	...
	public String updatePaymentStatusDate(HttpServletRequest request,HttpServletResponse response){	
		...
		try {
			...
			//只有在本地需要有更新项的时候采取查中间表更新--即状态为等待状态("A")
			if(loglist.size()>0){
	...
	private String  updatePaymentStatus(ResultSet  resultset) throws Exception{
			...
			if(resultset!=null){
				...
				while(resultset.next()){
					...
					chargelist=this.tableService.findEntityByProperties(ContractFundFundCharge.class, chargemap);
						if(loglist.size()>0){
							log0=loglist.get(0);
				...
				if(loglist0.size()>0){
					...
					if(chargelist0.size()>0){
						tableService.updateAllEntities(chargelist0);
					}
	...
	/**
	 * 查出来比较更新状态，此方法遇到数据比较多的时候容易内存溢出，比较low
	 * @param loglist
	 * @param resultset
	 * @return
	 * @throws Exception
