...
{   miniui_ext.saveIncludeData("tabDetails");
    ...
    var fundplanold = mini.get("fund_plan_old").getData();
    var totalcautionmoney = 0;//overmoney
    for(var i=0; i<fundplanold.length ;i++){
    	if("feetype16" == fundplanold[i].feetype){
    		totalcautionmoney += parseFloat(fundplanold[i].overmoney);
    	}
    }
    var pmoney = mini.get("caution_money_refund_detail").getData();
    var totalcautionmoney2 = 0;//overmoney
    for(var i=0; i<pmoney.length ;i++){
    	totalcautionmoney2 += parseFloat(pmoney[i].factmoney);
    }
    if(totalcautionmoney<totalcautionmoney2){mini.alert("抵扣金额合计大于保证金计划,请检查数据后操作");return false};
...
<div id="contract_change_form">
	...
	<div id="tabDetails" class="mini-tabs" activeIndex="0" onactivechanged="changTab" style="width: 100%;">
	    <div title="租金回笼计划" name="rent_income_plan" iconCls="icon-node">
			<jsp:include page="comm/rent_income_plan.jsp" ></jsp:include>
	    </div>
...
<div id="id_window_fund_offset" class="mini-window"  closed="true" modal="true" title="可抵扣的保证金" style="display:none;width:1000px;padding-top:20px;">
<jsp:include page="comm/fund_guarance_info.jsp" ></jsp:include>
</div>	
<script>
	jQuery(function() {
