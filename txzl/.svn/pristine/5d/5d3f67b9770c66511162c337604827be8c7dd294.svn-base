...
public class ComboboxServiceImpl implements ComboboxService {
	...
	public JSONArray getComboboxJson(String tableId, String filterId) throws Exception {
		...
		ReportContent table = this.reportDao.findEntityByID(ReportTable.class, tableId);
		if(table==null){
			table = this.reportDao.findEntityByID(ReportChart.class, tableId);
		}
		...
		if (filter != null) {
			...
			jsArray = comboBox.getJson(table, filter);
		}
