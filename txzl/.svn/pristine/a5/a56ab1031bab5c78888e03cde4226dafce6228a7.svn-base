...
import com.tenwa.business.entity.SystemLanguage;
...
public class PermissionUtil
{
	...
	private static Log			log			= LogFactory.getLog(PermissionUtil.class);
	private static BaseService	baseService	= (BaseService) WebUtil.getApplicationContext().getBean("baseService");
    //清除过期用户
	public static void clearExpireLoginInfos() throws Exception{
		List<LoginInfo> li_list = baseService.findResultsByHSQL("select li from LoginInfo li");
		for(LoginInfo loginInfo : li_list){
			//String now = DateUtil.getSystemDateTime();
			//String expireTime = loginInfo.getExpireTime();
			//if(now.compareTo(expireTime)>0)
			{
				baseService.removeEntity(loginInfo);
			}
		}
	}
	...
	private static void getUserMenusJsonObject(String username, Set<Menu> userMenus, Menu node, JSONObject jsonRootObject,
			JSONArray jsonArr,String firstLevelMenuId,Map<String,Set<QuickUserMenu>> quickUserMenusMap,Locale locale,List<Map<String,String>> ownedLeafMenus) throws JSONException, Exception
	{
		if (null == node)
			return;
		...
		boolean isHasChildren = node.isHasChildren();
 		String state  = isHasChildren ? "closed":"open";
		...
		if(isHasChildren)
		{
			...
			for (Menu mn : childrenNodes)
			{
				...
				getUserMenusJsonObject(username, userMenus, mn, null, jsonArray,firstLevelMenuId,quickUserMenusMap,locale,ownedLeafMenus);
			}
		}else{
			Map<String,String> ownedLeafMenu = new HashMap<String,String>();
			ownedLeafMenu.put("id", StringUtil.nullToString(node.getId()));
			ownedLeafMenu.put("pid", StringUtil.nullToString(pid));
			// modify by tracywindy 国际化菜单
			ownedLeafMenu.put("name",StringUtil.nullToString(text));
			ownedLeafMenu.put("text",StringUtil.nullToString(text));
			//.put("url", StringUtil.nullToString(node.getUrl()))
			ownedLeafMenu.put("url", StringUtil.nullToString(node.getParameterizedUrl()));
			ownedLeafMenus.add(ownedLeafMenu);
		}
	}
	...
	// 缓存所有用户的树形菜单
	private static Map<Locale, Map<String, String>> getAllUserMenusJsonString(Map<String, Set<Menu>> allUserMenus, Menu rootMenu,Map<String,Set<QuickUserMenu>> quickUserMenusMap,Map<String, List<Map<String, String>>> userOwnedLeafMenus )
			throws JSONException, Exception
	{
		Map<Locale, Map<String, String>> localeUserMenusJsonStringMap = new HashMap<Locale, Map<String, String>>();
		for (LocaleEnum localeEmnu : LocaleEnum.values()) {
			...
			localeUserMenusJsonStringMap.put(locale, userMenusJsonStringMap);
			for (String username : allUserMenus.keySet()) {
				List<Map<String,String>> ownedLeafMenus = new ArrayList<Map<String,String>>();
				userOwnedLeafMenus.put(username, ownedLeafMenus);
				Set<Menu> userMenus = allUserMenus.get(username);
				JSONObject rootJsonObj = new JSONObject();
				getUserMenusJsonObject(username, userMenus, rootMenu, rootJsonObj, null,"",quickUserMenusMap, locale,ownedLeafMenus);
	...
	// 缓存所有用户拥有的所有树形菜单
	public static void cachedAllUserTreeMenus(final boolean isUpdate) throws Exception
	{
		baseService.getBussinessDao().getHibernateTemplate().execute(new HibernateCallback<Map<String, String>>() {
			...
			public Map<String, String> doInHibernate(Session session)
				...
				for (User user : users) {
					...
					PermissionUtil.getUserMenus(user, userMenus);
					PermissionUtil.getUserResources(user, userResources);
					PermissionUtil.getUserActions(user, userActions);
					PermissionUtil.getUserWorkflowStarts(user, userWorkflowStarts);
					PermissionUtil.getUserWorkflowViews(user, userWorkflowViews);
				...
				Map<String, List<Map<String, String>>> userOwnedLeafMenus = new HashMap<String, List<Map<String, String>>>();
				...
				Map<Locale, Map<String, String>> userOwnedMenusJson = null;
				
				try {
					userOwnedMenusJson = getAllUserMenusJsonString(userMenus, rootMenu,quickUserMenusMap,userOwnedLeafMenus );
				} catch (JSONException e) {
					e.printStackTrace();
				} catch (Exception e) {
					e.printStackTrace();
				}
				//WebUtil.setUserOwnedMenusJson(userOwnedMenusJson);
				WebUtil.setPermissionCaches(userOwnedMenusJson, userOwnedMenus,userOwnedLeafMenus, userOwnedResources, userOwnedActions, userOwnedWorkflowStarts, userOwnedWorkflowViews, resourceOwnedUsers, userOwnedWorkflowStartSqlIdsStr, userOwnedWorkflowViewSqlIdsStr);
				ResourceUtil.setNeedUpdatePermissionCache(false);
				if(isUpdate){
					RemoteUtil.remotePermissionSynchronized(userOwnedMenusJson, userOwnedMenus,userOwnedLeafMenus, userOwnedResources, userOwnedActions, userOwnedWorkflowStarts, userOwnedWorkflowViews, resourceOwnedUsers, userOwnedWorkflowStartSqlIdsStr, userOwnedWorkflowViewSqlIdsStr);
				}
				return null;
			}
		});
	...
	{
		Map<String, String> allUserTreeMenus = WebUtil.getUserOwnedMenusJson().get(locale);
	 ...
	 * 系统初始化，数据库记录当前系统的语言
	 */
	public synchronized static void saveSystemLang(Locale currentLocale) throws Exception{
		List<SystemLanguage> lists =  baseService.findEntities(SystemLanguage.class);
		SystemLanguage lang = null;
		if(null != lists && 0 < lists.size()){
			lang =  baseService.findEntities(SystemLanguage.class).get(0);
		}
		if(null == lang){
			lang = new SystemLanguage();
		}
		if(currentLocale.getLanguage().equals("en")){
			lang.setSystemLocal(LocaleEnum.en_US);
		}else{
			lang.setSystemLocal(LocaleEnum.zh_CN);
		}
		baseService.saveOrUpdateEntity(lang);
	}
}
