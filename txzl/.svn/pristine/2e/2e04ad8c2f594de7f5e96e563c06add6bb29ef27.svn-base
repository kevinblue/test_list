...
function NTKO_OCX_OpenDoc(fileUrl)
{
	...
	OFFICE_CONTROL_OBJ.beginOpenFromURL(fileUrl,false,false);
}
...
function saveFileToUrl()
{
	...
	var fileName = document.all("fileid").value;
	var result  ;
	IsFileOpened=IsFileOpened||true;
	if(IsFileOpened)
	{
		...
		if(result=="编辑成功"){
			 $.ajax({
		         type: "post",
		         url:'${pageContext.request.contextPath}/acl/recordFileSaveState.acl',
		         data: {fileid:id},
