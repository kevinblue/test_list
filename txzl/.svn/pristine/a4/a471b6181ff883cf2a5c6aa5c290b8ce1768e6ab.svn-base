...
jQuery(function(){
	...
	tenwa.createTable({
		id: "contract_ancillary_wordinfo",
		renderTo: "id_table_contract_ancillary_wordinfo",
		width: '100%',
		height: 300,
		lazyLoad: false,
		title: "合同清单",
		...
		xmlFileName : '/eleasing/workflow/proj/proj_credit/contract_sign_detail_list.xml',
			params : {
				filekey :"${requestScope['contract_info.id']}"==""?mini.get("conditionContractId").getValue():"${requestScope['contract_info.id']}",
				modelname : '合同管理'
			},
		columns: [
			{type: 'indexcolumn'},
...
function showOperator(e){
	...
	var base1 = "<a href='javascript:void(0);' onclick='PreviewOffice(\""+id+"\",\""+ e.record.filename+"\")'>"+"预览</a>";
	...
	var tempright="2"
	//权限控制
	var authenticationstr = base1+"&nbsp;&nbsp;|&nbsp;&nbsp;"+base2
/* 	if(showTools){
...
function contractlistuploadFile() {
	var uploadutil = new uploadUtil({
			...
			tableid : "contract_ancillary_wordinfo",
			modelname : '合同管理',
			isAttachment:true,
...
<div id="id_table_contract_ancillary_wordinfo"></div>