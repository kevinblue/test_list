...
var showUploadFilesList = function(e) {
	...
	if(!fileListUUIDStr){
		...
		var activities = (e.record.attachmentFileUploadInfoDetailUploadActivities).split(splitStr); 
		var fileTypeListArr = [];
		//仅适用于miniui
		...
		for ( var i = 0; i < fileUUIDListArr.length; i++) {
			...
			if (!e.sender.isReadOnlyMini) {
				if (window.currentTaskActivityDetailId && (window.currentTaskActivityDetailId == fileAdListArr[i])) {/*<img border=0 align='absmiddle' width=14  src='" + getRootPath() + "/images/icon_delete.gif'/> style='position:relative;top:-2px'*/
					tempValue += "<td style='border:0px solid #FFF;width:40px;'><a onclick='removeUploadFile(\"" + e.sender.id + "\",\"" + fileUUIDListArr[i] + "\");' style='color:red;' title='删除' href='javascript:void(0);'>[删除]</a></td>";
				}
			}
			tempValue += "<td style='border:0px solid #FFF;'><img style='position:relative;top:-2px' align='absmiddle' width=14  src='" + getRootPath() + "/images/attach.gif'/>";
			tempValue += "<a href='javascript:void(0)' onclick='workFlowShowFile(\""+fileUUIDListArr[i]+"\",\""+ fileNameListArr[i]+"\")'>"+"预览</a> &nbsp;|&nbsp;"+"<a style='text-decoration:none;' onclick='downloadUploadFile(\"" + fileUUIDListArr[i] + "\");' title='"+workFlowlocale['DownloadFile']+"' href='javascript:void(0);'>" + fileNameListArr[i] + "</a></td>";
			tempValue += "<td style='border:0px solid #FFF;'>【"+workFlowlocale['uploadTime']+"：" + fileTypeListArr[i] + "】</td>";
			tempValue += "<td style='border:0px solid #FFF;'>【"+workFlowlocale['UploadActivity']+"：" + activities[i] + "】</td>";
...
//预览
function workFlowShowFile(filedict,filename){
	        ...
	        success:function(res){
			    ...
			    if(datas && datas.length > 0){
			    	...
			    	jQuery.each(datas, function(index, appImage){
			    		var img = workFlowAttachmentImageDiv(appImage,filedict);
			    		if(img){
...
function workFlowAttachmentImageDiv(appImage,filedict){
	var $div = jQuery('<div/>');
	if(appImage){
		...
		$image.attr("src", getRootPath()+"/acl/qeryCustfile.acl?filedict="+filedict);
		$image.appendTo($div);
	}
	return $div;
}
...
var apAttachmentFileUpload = function(config) {
	...
	var tableConfig = {
		...
		columns: [
			...
			{field : 'attachmentFileDictFileName', header : workFlowlocale['fileName'], width : '120'},
			...
			{field : 'operOne', header : workFlowlocale['Upload'], align : 'center', width : '100',
				renderer : function(e) {
					if (!identifierOne) {
			...
			{field : 'operTwo', header : workFlowlocale['UploadFileDetail'] ,width: 'auto',
				renderer: function(e){
					return showUploadFilesList(e);
				}
			}//文件清单
			,
			{field : 'remark', header : workFlowlocale['UploadFileDesc'],width: '150',
				renderer: function(e){
...
function uploadAttachmentFileCallBack(msg) {
	isSuccessFlag = true;
	if (mini.get("id_workflowhisAttachment")) {
	...
	if (mini.get("quotation_scheme")) {
		mini.get("quotation_scheme").reload();
	}
    if (mini.get("credit_report_list")) {
		mini.get("credit_report_list").reload();
	}
    if (mini.get("due_diligence_report2")) {
		mini.get("due_diligence_report2").reload();
	}
    if (mini.get("contract_word")) {
		mini.get("contract_word").reload();
	}
    if (mini.get("contract_ancillary_wordinfo")) {
		mini.get("contract_ancillary_wordinfo").reload();
	}
    if (mini.get("onhire_report")) {
		mini.get("onhire_report").reload();
	}
    if (mini.get("asset_classify")) {
		mini.get("asset_classify").reload();
	}
/*	
	if (mini.get("proj_risk_list")) {
		mini.get("proj_risk_list").reload();
	}
	if (mini.get("contract_word")) {
		mini.get("contract_word").reload();
	}
	...
	if (mini.get("assets_sign_word")) {
		mini.get("assets_sign_word").reload();
	}
	
	if (mini.get("patrol_report_list")) {
		mini.get("patrol_report_list").reload();
	}*/
	mini.alert(msg);
...
//批量下载生成文件
function batchDownloadBaseFileByIds(table, batchfilename) {
	var attchmentDetailIdStr = "";
	var include_file_ids = "";
	var checkedRowsData = table.getSelecteds();
	var len = checkedRowsData.length;
	if (len < 1) {
		...
		for(var i = 0; i < len; i++){
			attchmentDetailIdStr += (checkedRowsData[i].id + ",");
		}
		attchmentDetailIdStr = attchmentDetailIdStr.replace(/,$/g,"");
	}
	if ("" == attchmentDetailIdStr) {
		mini.alert("请选择要批量下载的资料！");
		return;
	...
	batchForm = document.createElement("form");
	with (batchForm) {
		action = getRootPath() + "/attachmentfile/downloadBatchAttachmentFilesByIds.action?browserType=" + SysBrowser.getBrowser().toLowerCase();
		target = table.name_exportExcelFormIFrame;
		style.display = "none";
		method = "post";
		innerHTML = "<input type='hidden' name='attchmentDetailBatchDownloadIdStr' value='" + attchmentDetailIdStr + "'/>" + "<input type='hidden' name='batchDownloadAttachmentFileTitle' value='" + batchfilename + "'/>";
	}
	document.body.appendChild(batchForm);
	batchForm.submit();
	return false;
