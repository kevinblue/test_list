...
jQuery(function(){
	...
	tenwa.createTable({
		id: "seal_accessories_report_list",
		renderTo: "id_table_seal_accessories_report_list",
		width: '100%',
		height: 200,
		lazyLoad: false,
		title: "",
...
//删除生成的文件。把文件注为无效    
 function dropCreateFile(rowDatas){
    ...
    mini.mask({el: document.body,cls: 'mini-mask-loading',html: '正在删除公章使用相关附件 请稍等...'});
...
function uploadFile() {
	var uploadutil = new uploadUtil({
			...
			modelname : '公章使用相关附件',
			isAttachment:true,
			attachmentParams:getAttachmentParams('root.FileType.Officialseal.01','${currentProcessInstanceDBID}'),
			parames : {
				flowUnid : flowUnid,
...
function reloadcustcontactfile(message){
	...
	mini.unmask(document.body);
	mini.get("seal_accessories_report_list").reload();
	loadcustomworkflowattachment();
}
</script>
<div id="id_table_seal_accessories_report_list"></div>