...
public class StrTools {
	...
	public static String getFileNameByFormual(String filename, Map<String, Object> model) {
		...
		while (m.find()) {
			...
			key = key.replace("}", "");
			if(key.indexOf("[")>0){
				 String arryName=key.substring(0, key.indexOf("["));
			     String lastName=key.substring(key.indexOf("["));
			     Matcher mindex = pindex.matcher(lastName);
			     List cindex=new ArrayList();
			     while(mindex.find()){
			    	 String temp= mindex.group(1).toString();
			    	 temp = temp.replace("[", "");
					 temp = temp.replace("]", "");
					 cindex.add(temp);
			     }
			     if(model.containsKey(arryName)){
				 if(cindex.size()==1){
						List tlist=(List)model.get(arryName);
						if(tlist.size()+1>=Integer.valueOf(cindex.get(0).toString())){
							value=StringUtil.nullToString(tlist.get(Integer.valueOf(cindex.get(0).toString())));
						}
				 }else if( cindex.size()==2 ){
					   List tlist=(List)model.get(arryName);
					   if(tlist.size()+1>=Integer.valueOf(cindex.get(0).toString())){
						   List clist=(List)tlist.get(Integer.valueOf(cindex.get(0).toString()));
						   if(clist.size()+1>=Integer.valueOf(cindex.get(1).toString())){
                           value=StringUtil.nullToString(clist.get(Integer.valueOf(cindex.get(1).toString())));
						   }
					  }
				 }}
			}else{
				if (model.containsKey(key)) {
					 value=StringUtil.nullToString(model.get(key));
				}
			}
			tempname = tempname.replace(m.group(1).toString(), value);
