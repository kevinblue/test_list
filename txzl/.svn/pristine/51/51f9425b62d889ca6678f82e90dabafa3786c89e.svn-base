...
public class FundRentPlanIrr implements Comparable<FundRentPlanIrr> {
	...
	public static void checkIrrFundRentPlan(FundRentPlanIrr irrBean,int index) throws Exception{
		...
		double   rentPlancheck   =new BigDecimal(irrBean.getRent()).doubleValue()-(new BigDecimal(irrBean.getBusinesscorpus()).add(new BigDecimal(irrBean.getBusinessinterest()))).doubleValue();
		rentPlancheck = (double)Math.round(rentPlancheck*100)/100;
		if(irrBean.getRentList() == null ){
			...
			throw new BusinessException("第"+index+"行的计划日期格式不正确！正确格式为yyyy年MM月dd日！");
		}else if(irrBean.getYearrate() != null && !irrBean.getYearrate().matches(rateRegex)){
			throw new BusinessException("第"+index+"行的年利率格式不正确！正确格式为小树点后最多保留六位！");
		}else if(rentPlancheck<-0.01 || rentPlancheck>0.01){
