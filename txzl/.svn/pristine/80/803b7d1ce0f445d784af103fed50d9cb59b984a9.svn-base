   ...
   <script type="text/javascript">
	   window.onresize = function(){
		...
		function nextRejectChecked(checkName,flag){
		     ...
		     //modify by zhangc 
		     jQuery("#id_nextRouteChoseWindow input[parent_name^='next_chose_']").each(function(i,e){
				 var obj = jQuery(this);
				 var parent_name = obj.attr('parent_name');
				 if(parent_name != checkName){
		...
		function toggleAdviseDisplay() {
			getLazyLoadedObj("advise", jQuery("#id_workflowAdviseContainer"), function(){
				if(currentSavedAdvise /*&& !isCompletedTask*/)  {
			    	addCommonAdivse(currentSavedAdvise,currentSavedResult,currentShowSavedResult);
				}
