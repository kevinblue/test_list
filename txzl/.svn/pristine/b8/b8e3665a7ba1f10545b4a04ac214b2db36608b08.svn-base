...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
			id: "grace_plan_modify",
			renderTo: "id_grace_plan_modify",
			...
			data: mini.decode('${empty json_grace_plan_modify_str ? "[]" : json_grace_plan_modify_str }'),
			tools : [ 'edit','-',{
				html : '更新现金流',
				plain : true,
				iconCls : 'icon-redo',
				handler : function(miniTable, buttonText) {
					...
					var graceplan = mini.get("grace_plan_modify").getData();
					var newplan =[];
					 ...
					 $.ajax({
				        ...
				        success: function (text) {
				        	...
				        	if(result.updateinfo == '成功'){
				        		...
				        		$("#id_json_grace_plan_modify_str").val(mini.encode(graceplan));
			...
			removeOperCallBack:function(miniTable){
				$('#id_json_grace_plan_modify_str').val(mini.encode(miniTable.getData()));
			},
...
<div id="id_grace_plan_modify"></div>