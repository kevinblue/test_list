...
{
	if (miniui_ext.submitFormValidation(["bussiness_five_category_form"]) == false) return false;
...
<div id="contract_change_form">
	...
	<div id="tabDetails" class="mini-tabs" activeIndex="0" onactivechanged="changTab" style="width: 100%;" bodyStyle="border:0px;">
		<div title="五级分类" name="bussinessfivecategory" iconCls="icon-node">
			<jsp:include page="comm/bussiness_five_category.jsp" ></jsp:include>
		</div>
	    ...
	    <%-- <div title="财务报表" name="custFinancialReport" iconCls="icon-cut">
				<jsp:include page="comm/cust_finance_report_newnew.jsp">
				<jsp:param value="true" name="isView"/>
				</jsp:include>
		</div>  --%>
		<%-- <div title="租赁物明细" name="proj_equip" iconCls="icon-cut" >
		 <jsp:include page="comm/proj_equip_detail.jsp" >
		 <jsp:param value="true" name="isView"/>
		 </jsp:include>
	    </div>
		<div title="担保单位信息" name="proj_guarantee_detail" iconCls="icon-cut">
	     <jsp:include page="comm/proj_guarantee_detail.jsp" >
	     <jsp:param value="true" name="isView"/>
	     </jsp:include>
	    </div> --%>
	  <%--   <div title="抵质押物信息" name="contract_guarantee_equip" iconCls="icon-node">
	    ...
	    <%-- <div title="签约方" name="contract_supplier" iconCls="icon-node">
		 ...
		 <div title="生成资产分类认定表" name="quotation_scheme" iconCls="icon-node" >
			<jsp:include page="comm/assets_five_category_confirm.jsp" ></jsp:include>
	     </div>  
	</div>
