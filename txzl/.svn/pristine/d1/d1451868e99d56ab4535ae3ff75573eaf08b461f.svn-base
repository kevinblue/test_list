...
jQuery(function(){
	...
	var tools= [{
		...
		plain : true,
		iconCls : 'icon-add',
		handler : function(miniTable, buttonText) {uploadFile4();}
	},'-','remove'];
	...
	tenwa.createTable({
		id: "credit_report_list4",
		renderTo: "id_table_credit_report_list4",
		width: '100%',
		height: 400,
		lazyLoad: false,
		title: "",
...
function uploadFile4() {
	...
	var uploadutil = new uploadUtil({
			...
			attachmentParams: {
				...
				attachmentFileDictId : 'FinancialAdjustment.01',//1、财务尽调报告
			...
			jscallback : 'reloadcustcontactfile4'
		});
	uploadutil.createFileAndShow();
}
function reloadcustcontactfile4(message){
	if (mini.get("id_customworkflowattachment")) {
		mini.get("id_customworkflowattachment").reload();
		mini.get("id_workflowhisAttachment").reload();
	}
	...
	mini.unmask(document.body);
	mini.get("credit_report_list4").reload();
}
</script>
<div id="id_table_credit_report_list4"></div>