...
public class QuartzJobPaymentUpdate implements Job {
	...
	private void updatePaymentStatus(ResultSet  resultset,BaseService baseService) throws Exception{
		...
		List<ContractFundPaymentInterfaceLog>  loglist=null;
		List<ContractFundFundCharge>  chargelist0=new ArrayList<ContractFundFundCharge>();
		List<ContractFundFundCharge>  chargelist=null;
		if(resultset!=null){
			ContractFundPaymentInterfaceLog  log0=null;
			ContractFundFundCharge  charge0=null;
			while(resultset.next()){	
				...
				chargemap.put("numUuid",loglist.get(0).getNumUuid());
				chargelist=baseService.findEntityByProperties(ContractFundFundCharge.class, chargemap);
					if(loglist.size()>0){
						log0=loglist.get(0);
						charge0=chargelist.get(0);
						//返回付款状态，如果失败会返回错误原因
						if("S".equals(paystatus)||"F".equals(paystatus)){
							log0.setPaymentStatus(paystatus);
							log0.setErrorReason(reason);
							//付款成功的时候才更新  金风实际会计处理日
							if("S".equals(paystatus)){
								log0.setLastUpdate(lastupdate);
								charge0.setFactDate(lastupdate.substring(0, 10));
							}
						}else{
						...
						loglist0.add(log0);
						chargelist0.add(charge0);
					}		
			}
			if(loglist0.size()>0){
				baseService.updateAllEntities(loglist0);	
				if(chargelist0.size()>0){
					baseService.updateAllEntities(chargelist0);
				}
				saveRemindPayment(loglist0,baseService);
			}
	 ...
	 * @throws Exception
	 */
	private  void  saveRemindPayment(List<ContractFundPaymentInterfaceLog>  loglist,BaseService baseService) throws Exception{
		...
		for(int i=0;i<loglist.size();i++){
			...
			DictionaryData dda=baseService.findEntityByID(DictionaryData.class, "msgtype.1");	
			bm.setMsgType(dda);
			bm.setFromUser(user);
			bm.setCreator(user);
