...
public class RentCalculateUtil {
	...
	public static List<RentPlanInfo> calculateForSameCorpusForSpecial(CalculationConditionImpl condition,SpecialRulesBean specialRulesBean,ConditionBean cb,int startList) throws Exception {
		...
		List<RentPlanInfo> rentPlanList = new ArrayList<RentPlanInfo>();
		...
		for (int i = startList>=specialRulesBean.getStartList()?startList:specialRulesBean.getStartList(); i <= specialRulesBean.getEndList(); i++) {
			...
			long days = 0;
			if(i == 1){//第一期
				days = DateTools.getDateDiff(rentPlan.get(i-1).getEndDate(),condition.getStartDate());
			}else{
				days = DateTools.getDateDiff(rentPlan.get(i-1).getEndDate(),rentPlan.get(i-2).getEndDate() );
			}
			tmpRate=new BigDecimal(specialRulesBean.getYearRate()).divide(new BigDecimal("100"), Scale.RATE_SCALE, BigDecimal.ROUND_HALF_EVEN).divide(new BigDecimal(360),20, BigDecimal.ROUND_HALF_EVEN).multiply(new BigDecimal(days));
			BigDecimal interest = leaseAmtRemain .multiply(tmpRate).setScale(Scale.INTEREST_SCALE, BigDecimal.ROUND_HALF_UP);
			if(condition.getPeriodType() == 0 && i==1){//期初
