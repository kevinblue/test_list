...
jQuery(function(){
	...
	var tools= [{
		...
		plain : true,
		iconCls : 'icon-add',
		handler : function(miniTable, buttonText) {uploadFile3();}
	},'-','remove'];
	...
	tenwa.createTable({
		id: "credit_report_list3",
		renderTo: "id_table_credit_report_list3",
		width: '100%',
		height: 400,
		lazyLoad: false,
		title: "",
...
function uploadFile3() {
	...
	var uploadutil = new uploadUtil({
			...
			attachmentParams: {
				...
				attachmentFileDictId : 'LegalRegulation.01',//1、法律尽调报告
			...
			jscallback : 'reloadcustcontactfile3'
		});
	uploadutil.createFileAndShow();
}
function reloadcustcontactfile3(message){
	if (mini.get("id_customworkflowattachment")) {
		mini.get("id_customworkflowattachment").reload();
		mini.get("id_workflowhisAttachment").reload();
	}
	...
	mini.unmask(document.body);
	mini.get("credit_report_list3").reload();
}
</script>
<div id="id_table_credit_report_list3"></div>