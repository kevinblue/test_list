...
//		}
	...
	*/
	public void saveOrUpdateTrigger(String triggerName, String jobName,String jobRecovery,String description, String cronExpression,String startDate,String durableDays,Map<String,Object> dataMap) throws Exception 
	{
		...
		CronExpression ce = new CronExpression(cronExpression);
		cronTrigger.setCronExpression(ce);
		cronTrigger.setMisfireInstruction(Integer.parseInt(jobRecovery));
	...
	public void updateTrigger(String cronexpression, String triggername)
		...
		getScheduler().unscheduleJob(triggername, QuartzJobMsg.MSG_GROUP_NAME);
		try
		{
	...
	@Override
	public void updateTriggerToNext() throws Exception {
		String quartzDetailsSql = "select tri.trigger_name,cron.cron_expression,tri.job_group from qrtz_triggers  tri "
				+ " left join qrtz_job_details job on job.job_name = tri.job_name "
				+ " left join qrtz_cron_triggers cron on cron.trigger_name = tri.trigger_name "
				+ " where job.requests_recovery  = 0  "
				+ " and tri.next_fire_time < (sysdate - to_date('1970-01-01','YYYY-MM-DD'))*24*60*60*1000 ";
		List<Map<String,Object>>  triggers =  this.baseService.queryListBySql(quartzDetailsSql);
		for(Map<String, Object> trigger : triggers){
			String triggerName = (String)trigger.get("trigger_name");
			String cronExpression = (String)trigger.get("cron_expression");
			String jobGroup = (String) trigger.get("job_group");
			Calendar calendar  = Calendar.getInstance();
			Date now = calendar.getTime();
			calendar.add(Calendar.YEAR, 2);
			Date deadLine = calendar.getTime();
			CronTrigger cronTri = new CronTrigger(triggerName, jobGroup, cronExpression);
			List<Date> fultureDates =  TriggerUtils.computeFireTimesBetween(cronTri, null, now, deadLine);
			Date nextFireTime =  fultureDates.get(0);
			String triggerUpdateSql =  "update qrtz_triggers set next_fire_time = ? where trigger_name = ?";
			this.baseService.getBussinessDao().getJdbcTemplate().update(triggerUpdateSql,nextFireTime.getTime(),triggerName );
		}
		this.baseService.updateFlush();
	}
	public boolean removeMsgTrigger(String triggerName) throws Exception
	{ 
		try 
		{
		  getScheduler().pauseTrigger(triggerName, QuartzJobMsg.MSG_GROUP_NAME);//停止触发器
		  return getScheduler().unscheduleJob(triggerName, QuartzJobMsg.MSG_GROUP_NAME);//移除触发器
		} catch (SchedulerException e) {
		log.error(e.getMessage());
		throw new SchedulerException();
		}
	}
	public void resumeMsgTrigger(String triggerName) throws Exception
	{ 
		try 
		{
		   getScheduler().resumeTrigger(triggerName, QuartzJobMsg.MSG_GROUP_NAME);//重启触发器
		} 
		catch (SchedulerException e) 
		{
		   log.error(e.getMessage());
	...
	public void pauseMsgTrigger(String triggerName) throws Exception
	{ 
		try 
		{
		   getScheduler().pauseTrigger(triggerName, QuartzJobMsg.MSG_GROUP_NAME);//停止触发器
