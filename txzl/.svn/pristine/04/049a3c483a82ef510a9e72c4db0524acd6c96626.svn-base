(function(window){
	...
	/**
	 * 树转表td合并列
	 * @param rowIndex 行号 columnIndex 列号 cols 合并几列
	 * @param tableId 只读的table的id
	 */
	window['cowSpanTd'] = function(tableId,rowIndex,columnIndex,cols){
		var trs  = $('#'+tableId).find('table tr');
		for(var i = 0 ; i < trs.length;i++){
			if(i == rowIndex){
				var tds = jQuery(trs[i]).find('td');
				for(var j = 0 ; j < tds.length;j++){
					if(j == columnIndex-1){
						jQuery(tds[j]).attr('colspan',cols);
					}
				}
			}
		}
	...
	/**
	 * 树转表行的显示控制
	 * @param rowIndex 行号 columnIndex 列号
	 * @param tableId 只读的table的id
	 */
	window['showRowColum'] = function(tableId,rowIndex,columnIndex){
		var trs  = $('#'+tableId).find('table tr');
		for(var i = 0 ; i < trs.length;i++){
			if(i == rowIndex){
				var tds = jQuery(trs[i]).find('td');
	 ...
	 * @param tableId 只读的table的id
	 */
	window['hiddenRowColum'] = function(tableId,rowIndex,columnIndex){
		var trs  = $('#'+tableId).find('table tr');
		for(var i = 0 ; i < trs.length;i++){
			if(i == rowIndex){
				var tds = jQuery(trs[i]).find('td');
				...
				for(var j = 0 ; j < tds.length;j++){
					if(isInArray(j+1,index)){
						...
						if(input && input.length > 0 ){
							...
							if(type != "text"){
								html = "<label style='"+styleStr+"' value='"+value+"' dictId='"+dictId+"' columnId='"+columnId+"'  name='"+name+"'><span class='"+className+"'></span>" + tdText + "</label>";
							}
							jQuery(tds[j]).html('<div class="x-panel-tree2table-data-div">' + html + "</div>");
						}
						if(textarea && textarea.length > 0 ){
							var html = '';
							var value = textarea.text();//typeof(textarea.attr('value')) == 'undefined' ? textarea.text() : textarea.attr('value');
