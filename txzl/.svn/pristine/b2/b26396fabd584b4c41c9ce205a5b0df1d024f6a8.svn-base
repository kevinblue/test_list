...
<input  style="display:none;"	class="mini-textarea" id="proj_info.subchangecontent" name="proj_info.subchangecontent" value="${requestScope['proj_info.subchangecontent']}"></input>
...
<div id="onhire_contract_change_form" title="合同变更">
	...
	<input name="changeType"id="changeType" 	 style="display:none"     	class="mini-textbox" value="${requestScope['changeType']}"/>
	<div id="tabDetails" class="mini-tabs" activeIndex="0" style="width: 100%;" onactivechanged="changTab" >
		<div title="变更说明" name="contract_change_info" iconCls="icon-node">
			<jsp:include page="../contract_comm/contract_change_info01.jsp" ></jsp:include>
		</div>
		...
		<div title="其它实质性变更" name="contract_change_other_info" id="contract_change_other_info" iconCls="icon-node">
			<jsp:include page="comm/contract_change_other_info.jsp"></jsp:include>
		</div>
		<div title="(变更前)商务条件" name="business_condition_before" iconCls="icon-cut">
			<jsp:include page="../../reckon/rent_readonly/main_5_1.jsp" ><jsp:param value="true" name="isView"/></jsp:include>
		</div>
		<div title="变更信息" name="change_info" iconCls="icon-node">
			<jsp:include page="contract_planchange_info.jsp" ></jsp:include>
		</div>
		 <div title="(变更后)商务条件" name="business_condition_after" iconCls="icon-cut">
			<jsp:include page="../../reckon/rent_compare/main_5_1_2.jsp" ><jsp:param value="true" name="isView"/></jsp:include>
		</div> 
		<div title="租金计划修改" name="fund_rent_plan_modify" iconCls="icon-cut" showCollapseButton="true">
			<jsp:include page="../../reckon/rent_modify/main_5_1.jsp" ></jsp:include>
		</div>
		<div title="租后变更通知书" name="rent_change_notice" iconCls="icon-node" >
			<jsp:include page="create_rent_change_notice.jsp" ></jsp:include>
		</div>  
		...
		<%-- <div title="商务条款变更" name="business_condition" id="business_condition" iconCls="icon-node">
			<jsp:include page="comm/commerce_clause_change.jsp"></jsp:include>
		</div>
		<div title="租金计划变更" name="contract_supplier" id="contract_supplier" iconCls="icon-node">
			<jsp:include page="comm/rent_plan_change.jsp"></jsp:include>
		</div>
		 <div title="合同变更协议" name="contract_planchange_tab" id="contract_planchange_tab" iconCls="icon-cut">
			<jsp:include page="../contract_comm/contract_change_word1.jsp" ></jsp:include>
		</div>  --%>
	</div>
</div>
<script>
jQuery(function(){
	...
	var change_info = tabs.getTab("change_info");
	var business_condition_after = tabs.getTab("business_condition_after");
	var fund_rent_plan_modify = tabs.getTab("fund_rent_plan_modify");
	var changeType = mini.get("changeType").getValue().trim();
	if(changeType == "租金计划修改"){
		tabs.updateTab(change_info,{visible:false});
		tabs.updateTab(business_condition_after,{visible:false});
	}else{
		tabs.updateTab(fund_rent_plan_modify,{visible:false});
	}
