 ...
 * @info 合同现金流明细(临时)  临时表暂时不设置主外键    临时表不推荐使用
 * @Copyright 
 * Tenwa
 */
@Entity
@FieldName(name = "合同现金流明细(临时)")
@Table(name="CONTRACT_CASH_DETAIL_TEMP")
@JsonIgnoreProperties(ignoreUnknown=true,value = { "hibernateLazyInitializer" , "creator" ,"modificator"})
public class ContractCashDetailTemp implements CashDetail, Comparable<CashDetail> {
	...
	@FieldName(name="客户多次起租编号")//欧力士用于在按固定格式补0的现金流排序使用
