...
jQuery(function() {
		...
		seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
			new ApTable({
							...
							handler : function(miniTable, buttonText) {
								if(industryType=="风力发电"||industryType=="光伏发电"){
									...
									var fileTeplate=new fileTemplateConfig({
										isAttachment : true,
										attachmentParams:getAttachmentParams("CustomerReport.01","${currentProcessInstanceDBID}"),
										templateno : 'F-201703006',
										tableid : 'due_diligence_report2',
										modelname : '生成客户巡视报告',
				...
				xmlFileName:'/eleasing/workflow/proj/proj_credit/wind_farm_report_file_info.xml',
...
function getParamspatrol1(){
	...
	params["baseinfo.finbalance"] =mini.get("financing_balance").getValue();//融资余额（万元）
	...
	//检查方式
	params["label.checkmode"] =mini.get("checkMode").getText();
	...
	params["contract_patrol_info.visitPensonnel"] =mini.get("rawValue_contract_patrol_info.visitPensonnel").getValue();//
    ...
    var equipnames= mini.get('leasing_insurance_list').getData();
    var equipname="";
    for(var i=0;i<equipnames.length;i++){
    	if(i==equipnames.length-1){
    	equipname+=equipnames[i].equipname;
    	}else{
        equipname+=equipnames[i].equipname+',';
    	}
    }
    params["baseinfo.equipname"]=equipname;
    //抵押物
