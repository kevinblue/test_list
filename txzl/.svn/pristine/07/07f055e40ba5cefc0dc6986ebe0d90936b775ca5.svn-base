...
<!--提交按钮-->
<div id="__jbpmProcessButtonsToolbar" class="mini-panel" maskOnLoad="false" title="<spring:message code="Workflow"  text="流程"/>：${currentWorkFlowDisplayName}[${currentTaskName}][${currentProcessInstanceDBID}]" style="width: 100%;" bodyStyle="padding:2px 6px 2px 6px;">
	...
	<c:if test="${('true' ne isViewHistoryTask)}">
		...
		<c:if test="${('assignmentPending' eq currentRequestTaskType)}">
			...
			<a class="mini-button" onclick="__submitButtonDisplayText" id="__submitButtonDisplayText" iconCls="icon-ok">提交</a>
		</c:if>
		<c:if test="${('assignmentCompleted' eq currentRequestTaskType)}">
			...
			<a class="mini-button" onclick="__backButtonDisplayText" id="__backButtonDisplayText" iconCls="icon-undo">退回</a>
			</c:if>
