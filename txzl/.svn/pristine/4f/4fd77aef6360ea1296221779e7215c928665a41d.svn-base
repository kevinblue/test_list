...
<script type="text/javascript">
	...
	function workflowSubmitCallBack(buttonText){//返回true提交，返回false不提交 ，该回调函数可与用表单字段验证
		if (miniui_ext.submitFormValidation(["id_project_pre_vote"]) == false) return false;
        miniui_ext.saveIncludeData("tabDetailsBase");
		return true;
	}
	...
	function workflowNextRouteCallBack(buttonText,requestNextRoute){
		if(buttonText == "Submit"){
			if(mini.get("preisagree").getValue()=="通过"){
	...
	<div id="contract_approval_form">
		<div class="fillTableContainer">
			<table class="fillTable" cellspacing="0" cellpadding="0">
					...
					<tr>
						<!-- 评审委员会填写审批意见，审批状态 -->
						<td colspan=4> <jsp:include page="common/proj_pre_vote.jsp"></jsp:include></td>
					</tr>
			</table>
		</div>
