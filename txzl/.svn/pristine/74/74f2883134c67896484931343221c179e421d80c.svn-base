...
{   
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:999999999999999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:180px;font-size:12px;"></div>';   
    /*if(document.all)   
    {   
        str += '<iframe style="position:absolute;z-index:999999999999999;width:expression(this.previousSibling.offsetWidth);';   
...
//选择日期 → 由 www.ttkc.net 2006-06-25 添加
 ...
 {
	 strFormat = strFormat.replace(/HH/gi,"hh");
...
//取得HTML控件绝对位置
 Calendar.prototype.getAbsPoint = function (e){
   ...
   while(e = e.offsetParent){
	 ...
	 var el = this.dateControl.parentNode;
	 while(el)
	 {
		  if(el.className){
			  if( (el.className.indexOf("panel-body")>-1) && ((el.className.indexOf("window-body")>-1)||(el.className.indexOf("dialog-content")>-1))){
				  ...
				  x-=el.scrollLeft;
				  break;
			  }else if((el.className.indexOf("x-panel-tree2table-div")>-1)){
				  y-=el.scrollTop;
				  x-=el.scrollLeft;
				  break;
			  }
   ...
   {
  	 if(isParent(this.dateControl,workflowFormContainer))
  	 {
  	   x -= workflowFormContainer.scrollLeft;
	   y -= workflowFormContainer.scrollTop;
  	 }
   }
   return {"x": x, "y": y};
 };
...
//显示日历
 Calendar.prototype.show = function (dateObj, popControl) {
  ...
  if (popControl == null){
   ...
   var realX = (xy.x + leftX);
   var realY = (xy.y + topY + dateObj.offsetHeight+2);
   var clientHeight = document.documentElement.clientHeight;
   var dateContainerHeight = 203;
   if(this.dateFormatStyle.indexOf("hh:")>-1){
	   dateContainerHeight = 233;
   }
   if((realY+dateContainerHeight+2)>clientHeight){
	   realY = realY - dateContainerHeight - 2 - dateObj.offsetHeight;
   }
   this.panel.style.left = realX + "px"; //由www.ttkc.net 2007-02-11 修改 → 加入自定义偏移量
   this.panel.style.top =  realY + "px";
  ...
  this.dateControl.onblur = function(){calendar.onblur();if(typeof(this.blurEvent) =='function'){this.blurEvent();}
  };
   }
...
//以下由www.ttkc.net 2007-07-26 修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方
 ...
 function WdatePicker(obj,config){
	// {readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'}
	 SelectDate(obj,config.dateFmt?config.dateFmt:'yyyy-MM-dd');
 }
 //调用calendar.show(dateControl, popControl);
 //--> 
 /*
 ...
 <input type="text" value="" maxlength="100" id="Text4"  onclick="SelectDate(this,'yyyy年MM月dd日 hh时mm分ss秒')" readonly="true" style="width:265px;cursor:pointer" /><br />
 */