   ...
   private AttachmentFileService attachmentFileService ;
	...
	public String uploadAttachmentFile(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try {
			...
			String ajaxCallBackScript = "<script type='text/javascript'>window.parent.uploadAttachmentFileResultCallBack('"+e.getMessage()+"');</script>";
			this.ajaxReturn(response, ajaxCallBackScript);
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String uploadAttachmentFiles(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			String ajaxCallBackScript = "<script type='text/javascript'>window.parent.uploadAttachmentFileResultCallBack('"+(WebUtil.isEnglish() ? "upload error" : "上传失败" )+"！');</script>";
			this.ajaxReturn(response, ajaxCallBackScript);
		}
		return null;
	}
	...
	//批量下载生成文件
	@RequestMapping(value="/downloadBatchAttachmentFilesByIds.action")
	public String downloadBatchAttachmentFilesByIds(HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		Map<String,String> modelData = QueryUtil.getRequestParameterMapNoAjax(request);
		String attchmentDetailBatchDownloadIdStr = modelData.get("attchmentDetailBatchDownloadIdStr");
		String batchDownloadAttachmentFileTitle =  modelData.get("batchDownloadAttachmentFileTitle");
		String browserType = modelData.get("browserType");
		this.attachmentFileService.downloadBatchAttachmentFilesByIds(response,batchDownloadAttachmentFileTitle,attchmentDetailBatchDownloadIdStr,browserType);
		return null;
	}
