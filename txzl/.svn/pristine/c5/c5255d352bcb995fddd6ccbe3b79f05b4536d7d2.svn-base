...
public class InterestCalServiceImpl {
	 ...
	 * @return
	 */
	public List<String> getInterestList(List<String> rent_list, String leaseMoney, String calRate, String periodType, int sgrace, String Income_number_year, String endValue,String rateType) {
		...
		String preRate = new BigDecimal(calRate).divide(new BigDecimal(1200),20,BigDecimal.ROUND_HALF_UP).toString();
		// 得到宽限期利息
		...
		for (int i = grace; i < rent_list.size(); i++) {// 循环租金list
			...
			if ("1".equals(periodType) && i == grace) {// 第一期且是期初时
				...
				corpus_overage = new BigDecimal(corpus_overage).subtract(corpus).toString();
				
			} else {
				for(int j=0;j<12/Integer.parseInt(Income_number_year);j++){
					
					inte = new BigDecimal(corpus_overage).multiply(new BigDecimal(preRate)).add(inte).setScale(2, BigDecimal.ROUND_HALF_UP);
					if(j==12/Integer.parseInt(Income_number_year)-1){
						corpus = new BigDecimal(rent_list.get(i).toString()).subtract(new BigDecimal(corpus_overage).multiply(new BigDecimal(preRate))).setScale(2, BigDecimal.ROUND_HALF_UP);
					}else{
						corpus=new BigDecimal(corpus_overage).multiply(new BigDecimal(preRate)).multiply(new BigDecimal(-1)).setScale(2, BigDecimal.ROUND_HALF_UP);
					}
			...
			if (i == rent_list.size() - 1) {
				// 本金 --总的本金-以前的本金和-期末残值
				corpus = new BigDecimal(leaseMoney).subtract(new BigDecimal(corpus_total)).subtract(new BigDecimal(endValue));
				// 利息
	...
	public List<String> getInterestList(List<String> rent_list, String leaseMoney, String corpusCon, List<String> year_rate, String periodType, int sgrace, String Income_number_year, String endValue, String[] rentAdjustList,String rateAdjustType) {
		...
		// 得到宽限期利息
		int grace = sgrace;
		interests = getGraceInterest(year_rate, leaseMoney, Arrays.asList(rentAdjustList), grace, Income_number_year,rateAdjustType);
		leaseMoney = corpus_overage;// 9950
		for (int i = grace; i < rent_list.size(); i++) {// 循环租金list
			preRate = RateTools.getPreRate(year_rate.get(i), Income_number_year,rateAdjustType);
			// 起租类型  注意: 期初 数字 1 字符串 period_type_1  #分割线#  期末 数字0 字符串 period_type_0
			if ("1".equals(periodType) && i == grace) {// 第一期且是期初时
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<String> getInterestList(List<String> rent_list, String leaseMoney,ConditionBean cb, SpecialRulesBean  specialRulesBean, String periodType, int sgrace, String Income_number_year, String endValue,Boolean ... isRent) {
		...
		String preRate = new BigDecimal(specialRulesBean.getYearRate()).divide(new BigDecimal("100"), Scale.RATE_SCALE, BigDecimal.ROUND_HALF_EVEN).multiply(new BigDecimal(specialRulesBean.getRegularMonths())).divide(new BigDecimal(12),Scale.RATE_SCALE,BigDecimal.ROUND_HALF_EVEN).toString();
		String tmpRate=preRate;
		for (int i = specialRulesBean.getStartList(),j=0; i <= specialRulesBean.getEndList(); i++,j++) {// 循环租金list
			// 起租类型  注意: 期初 数字 1 字符串 period_type_1  #分割线#  期末 数字0 字符串 period_type_0
	 ...
	 * @return
	 */
	private List<String> getGraceInterest(List<String> year_rate, String corpus, List<String> l_rent_adjust, int grace, String income_number_year,String rateAdjustType) {
...
//		logger.info("财务利率：" + cb.getNoGraceIRR() + "总本金:" + leaseMoney + "计算利率：" + preRate);
	...
	@SuppressWarnings("unchecked")
	public List<String> getKnowingCorpusInterestList(List<String> corpus_list, String leaseMoney,ConditionBean cb, SpecialRulesBean  specialRulesBean, String periodType, int sgrace, String Income_number_year, String endValue,Boolean ... isRent) {
		...
		// 本金余额,
		corpus_overage = cb.getRemainCorpus().toString();
		String preRate = new BigDecimal(specialRulesBean.getYearRate()).divide(new BigDecimal("100"), Scale.RATE_SCALE, BigDecimal.ROUND_HALF_EVEN).divide(new BigDecimal(360),Scale.RATE_SCALE, BigDecimal.ROUND_HALF_EVEN).multiply(new BigDecimal(cb.getRateType())).multiply(new BigDecimal(specialRulesBean.getRegularMonths())).divide(new BigDecimal(12),Scale.RATE_SCALE,BigDecimal.ROUND_HALF_EVEN).toString();
		String tmpRate=preRate;
		for (int i = specialRulesBean.getStartList(),j=0; i <= specialRulesBean.getEndList(); i++,j++) {// 循环本金list
			if ("1".equals(periodType) && i == 1) {// 第一期且是期初时
				inte = "0";
			}else{
				// 利息
