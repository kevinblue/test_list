...
jQuery(function(){
	...
	var tools= [{
		...
		plain : true,
		iconCls : 'icon-add',
		handler : function(miniTable, buttonText) {uploadFile2();}
	},'-','remove'];
	...
	tenwa.createTable({
		id: "credit_report_list2",
		renderTo: "id_table_credit_report_list2",
		width: '100%',
		height: 400,
		lazyLoad: false,
		title: "",
...
function uploadFile2() {
	...
	var uploadutil = new uploadUtil({
			...
			attachmentParams: {
				...
				attachmentFileDictId : 'TechnicalRiskControl.01',//1、技术风控尽调报告
...
function reloadcustcontactfile2(message){
	...
	mini.unmask(document.body);
	mini.get("credit_report_list2").reload();
}
</script>
<div id="id_table_credit_report_list2"></div>