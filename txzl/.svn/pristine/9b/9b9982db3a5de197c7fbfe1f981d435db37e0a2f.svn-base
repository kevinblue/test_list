...
function showTable(title,financeid){
	...
	ajaxRequest({
		...
		success:function(response){
		    ...
		    if(temptitle=="资负表"){ trs=initGread(jsondata);}
...
<div id="tabDetails" class="mini-tabs"  activeIndex="0"  onactivechanged="onFinanceactivechanged" style="width: 100%;" bodyStyle="border:0px;">
	    <div title="资负表" name="297e072a57d574e40157d5a096680013" iconCls="icon-node">
			 <table id="297e072a57d574e40157d5a096680013" style="border-collapse: collapse; width: 100%" cellspacing="0"></table>
		</div>
		<div title="利润表" name="297e072a57d574e40157d59d6bc60008" iconCls="icon-node">
			  <table id="297e072a57d574e40157d59d6bc60008" style="border-collapse: collapse; width: 100%" cellspacing="0"></table>
		</div>
		<div title="现金流量表" name="297e072a57d574e40157d5a3d9820014" iconCls="icon-node">
			  <table id="297e072a57d574e40157d5a3d9820014" style="border-collapse: collapse; width: 100%" cellspacing="0"></table>
		</div>
		<div title="表外数据录入" name="297e072a57d574e40157d5a7eb790015" iconCls="icon-node">
			  <table id="297e072a57d574e40157d5a7eb790015" style="border-collapse: collapse; width: 100%" cellspacing="0"></table>
		</div>
</div>
 ...
 function initGread(jsondata){
	 ...
	 for(var i = 0;i<jsondata.length-4;i++){
		...
		//for(var j=0;j<subjectnames.length;j++){
			trs += "<th width='50'>"+subjectnames[0]+"</th>";
	   // }
		//拼接行的值
		for(var j = 2;j < values.length;j+=2){
			if(i==1||i==7||i==20||i==28){
				trs += "<td>" +values[j].split(".")[0] + "</td>";
			}else{
			   var temp=values[j]
		        temp=isNaN(temp)==false?temp:parseFloat(parseFloat(temp)*100).toFixed(2);
		         temp=isNaN(temp)==false?(temp=="NaN"?"":temp):temp+"%";
		        trs += "<td>" +temp + "</td>";
			}
		}
		trs += "</tr>";
	 ...
	 for(var i = 0;i<jsondata.length;i++){
			...
			trs += "<tr>";
			//for(var j=0;j<subjectnames.length;j++){
				trs += "<th width='50'>"+subjectnames[1]+"</th>";
		   // }
