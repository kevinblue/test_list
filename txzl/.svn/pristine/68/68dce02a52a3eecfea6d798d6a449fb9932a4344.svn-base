...
public class CalculationConditionImpl implements CalculationCondition {
	...
	private int remainLeaseTerm = 0; // 剩余租赁期限
...
*/
	...
	public void copyConditionBeanValues(ConditionBean cb) throws Exception {
		...
		this.leaseAmtDate = dateFormat.parse(cb.getLeaseAmtDate()); // 设备款支付日期
...
//		}
		this.remainLeaseTerm=cb.getRemainLeaseTerm();//剩余租赁期限
		this.incomeTimes = cb.getIncomeNumber(); // 还租期数
		if(cb.getIncomeNumberYear()!=null&&cb.getIncomeNumberYear()!=""){
			this.issueNumber = 12/Integer.parseInt(cb.getIncomeNumberYear()); // 年还租次数
		}	
		this.grace = cb.getGrace(); // 宽限期数，只收利息，由于本金不变所以利息相同
		// 起租类型  注意: 期初 数字 1 字符串 period_type_1  #分割线#  期末 数字0 字符串 period_type_0
		if(cb.getPeriodType().matches("[0-9]+")){
...
//		this.issueRateReCal = Integer.parseInt(cb.getIfretry()); // 是否复算，季付时，原有的和这个接口定义的意思正好相反
		...
		//this.yearRate = new BigDecimal(cb.getYearRate()).divide(new BigDecimal(100)).setScale(Scale.RATE_SCALE, BigDecimal.ROUND_HALF_UP); // 在天信项目 该字段无效 租赁年利率
		// **********************************计算现金流********************************************
		// 期初收入
