...
import com.tenwa.leasing.entity.proj.RecordFileSaveState;
import com.tenwa.leasing.entity.proj.ReserveRatioData;
import com.tenwa.leasing.service.cust.CustValidatorService;
import com.tenwa.leasing.serviceImpl.fileTemplate.readExcel;
...
public class CustValidatorController {
	...
	/*		String userid = model.get("userid");
			...
			if("second".equals(previewtype)){
				...
				downloadedFileFullPath = FileUtil.getFilePathString(ResourceUtil.getFileUploadDataPath()+"/wordFiles/" + downloadedFilePath);
	...
	//读取经评模型excel
	@RequestMapping(value = "/getExcelValue.acl")
	@ResponseBody
	public String getExcelValue(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		try{
			readExcel readexcel = new readExcel();
			String indexAndid = request.getParameter("indexAndid");
			String projectid = request.getParameter("projectid");
			String flowId = request.getParameter("flowUnid");
			JSONObject fileIndexAndId = new JSONObject(indexAndid);
			String contractWordDir = ResourceUtil.getWordFilesRelativeStorePath();
			String uploadDirPath = FileUtil.getFilePathString(ResourceUtil.getFileUploadDataPath()) + contractWordDir + "/";
		   ...
		   List<ReserveRatioData> rrdListNew = new ArrayList<ReserveRatioData>();
		      propertiesMap.clear();
		      propertiesMap.put("projId", projectid);
			...
			if(fileIndexAndId.length()==8){
				for(int i=0;i<8;i++){
					String ee = (String) fileIndexAndId.get(i+"");
					BaseFile bf = this.tableService.findEntityByID(BaseFile.class, ee);
					jsonArray = readexcel.getExcelValue(uploadDirPath+bf.getFileAddress(),i+"",projectid,flowId,bf.getFileName());
					this.tableService.saveOrUpdateEntitiesByJSONArrayString(EvaluationModelData.class, "["+jsonArray[0]+"]", null, "");
					this.tableService.saveOrUpdateEntitiesByJSONArrayString(EvaluationModelData.class, "["+jsonArray[1]+"]", null, "");
					this.tableService.saveOrUpdateEntitiesByJSONArrayString(ReserveRatioData.class, "["+jsonArray[2]+"]", null, "");
				}
			}else{
				return "0";
			}
			...
			e.printStackTrace();
			throw new Exception("文件数据异常，请检查EXCEL中的输出结果！");
		}
	}
	//记录经评模型文件编辑保存状态
	@RequestMapping(value = "/recordFileSaveState.acl")
	@ResponseBody
	public void recordFileSaveState(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		try{
		String fileid = request.getParameter("fileid");
		RecordFileSaveState recordfilesavestate = new RecordFileSaveState();
		recordfilesavestate.setFileId(fileid);
		recordfilesavestate.setSaveState("已保存");
		recordfilesavestate.setCreateDate(DateUtil.getSystemDate());
		recordfilesavestate.setCreator(SecurityUtil.getUserId());
		this.baseService.saveEntity(recordfilesavestate);
		}catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	@RequestMapping(value = "/getFundReceiptNo.acl")
	@ResponseBody
	public String getFundReceiptNo(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	System.out.println("这里测试一下=============================");
		try{
            String number="No."+createFundReceiptNum();
			return number;
		}catch (Exception e) {
			return "调用失败!["+e.getMessage()+"]";
		}
	...
	private synchronized String createFundReceiptNum() throws Exception {
		try{			
			String year=DateUtil.getSystemDate().substring(0, 4);
			String month=DateUtil.getSystemDate().substring(5, 7);
			//String day=DateUtil.getSystemDate().substring(8, 10);
			List<Map<String,Object>> numberlist=this.tableService.queryListBySql("select * from T_SERIAL_NUMBER where type_='租金收据号' ");
			if(numberlist!=null&&numberlist.size()>0){
				if(new BigDecimal(year).compareTo(new BigDecimal(numberlist.get(0).get("year_").toString()))!=0){
					this.tableService.updateBySql("update T_SERIAL_NUMBER set year_=?, month_=?, order_number_=? where  type_='租金收据号' ", Integer.parseInt(year),Integer.parseInt(month),0001);
					return year+"0001";
				}else{
					BigDecimal number=new BigDecimal(numberlist.get(0).get("order_number_").toString()).add(BigDecimal.ONE) ;
					String format=new DecimalFormat("0000").format(number);
					this.tableService.updateBySql("update T_SERIAL_NUMBER set  order_number_=? where  type_='租金收据号' ", Integer.parseInt(format));
					return year+format;
				}
			}else{
				this.tableService.updateBySql("insert into T_SERIAL_NUMBER ( id_,type_,year_,month_,order_number_) values(sys_guid(),'租金收据号',?,?,?)", Integer.parseInt(year),Integer.parseInt(month),0001);
				return year+"0001";
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	public String updatequartzJobHoliday(HttpServletRequest request,
		...
		try{
			...
			String id=model.get("id");
			String message=baseService.updatequartzJobHoliday(year,id);
	...
	//自动创建节假日编号
	@RequestMapping(value="/createHolidayNumber.acl")
	@ResponseBody
	public String createHolidayNumber(HttpServletRequest request,
			HttpServletResponse response) throws Exception{
		try{
			return baseService.createHolidayNumber();
		}catch (Exception e) {
			return "{message:'查询失败!["+e.getMessage()+"]'}";
		}		
	}
}
