...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
			id: "fund_pre_schedule",
			renderTo: "id_table_fund_pre_schedule",
			...
			tools: ['remove','-',
					...
					handler : function(miniTable, buttonText) {
				       ...
				       for(var i=0;i<cdates.length;i++){ 
						...
						var params=getPreParams();
						var fileTeplate=new fileTemplateConfig({
							templateno : 'F-201708013',
							tableid : 'fund_pre_schedule',
							modelname : '资金计划预实表',
...
//删除生成的文件。把文件注为无效
function dropPreCreateFile(rowDatas){
...
<div id="id_table_fund_pre_schedule"></div>