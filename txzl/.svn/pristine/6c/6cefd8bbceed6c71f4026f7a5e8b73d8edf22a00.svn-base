...
function loadReportChartData(queryObject){
	var filterMini = jQuery('#filter_'+queryObject.id);
	var width = jQuery('#chart_data_' + queryObject.id).width();
	...
	if(queryObject.paramValues){
		url += "&paramValue=" +secutiry.des.encrypt(queryObject.paramValues,'4860B1E27','7464DB1C804E','426A2405196')+"&browser"+SysBrowser.getBrowser();
	}
	...
	jQuery('#div_common_button_' + queryObject.id).css('display','block');
	if(queryObject.hasFilter){
    	jQuery('#div_showquery_button_' + queryObject.id).css('display','');
    }
	jQuery('#__modalmini-1').css('z-index','-1');
}
...
function showOrHiddenQueryArea(aLink,tableId,isChart){
	...
	if(status=='none'){
		...
		if(isChart){
			dateMini = jQuery('#chart_data_'+tableId);
			filterMini = jQuery('#chart_filter_'+tableId);
			dateMini = $('.FusionCharts');
		}else{
