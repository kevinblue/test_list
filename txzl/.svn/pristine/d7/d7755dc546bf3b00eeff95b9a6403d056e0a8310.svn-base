...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		...
		validateForm: function(miniTable, miniForm){
			...
			{
				mini.alert("本次付款金额不能大于计划可付款金额!("+comparemoney+")");
				return false;
			}
		  var purpose=getMiniEditFormField("put_current.purpose").getValue();
		  if(getLength(purpose)>25){
			  mini.alert("支付用途不能超过25个字符或8个汉字");
			  return false;
		  } 
		  var clientaccnum=getMiniEditFormField("put_current.clientaccnumber").getValue();
		  if(clientaccnum==""||clientaccnum==null){
			  mini.alert("对方收款账号不能为空！");
			  return false;
		  }
		  var clientaccount=getMiniEditFormField("put_current.clientaccount").getValue();
		  if(clientaccount==""||clientaccount==null){
			  mini.alert("对方收款账户不能为空！");
			  return false;
		  }
		  var bankshortname=getMiniEditFormField("put_current.depositbanktype").getValue();
		  if(bankshortname==""||bankshortname==null){
			  mini.alert("收款方银行简称不能为空！");
			  return false;
		  }
		  
		  var clientbankno=getMiniEditFormField("put_current.clientbankno").getValue();
		  if(clientbankno==""||clientbankno==null){
			  mini.alert("对方收款银行行号不能为空！");
			  return false;
		  }
		  var unionclientbankno=getMiniEditFormField("put_current.unionclientbankno").getValue();
		  if(unionclientbankno==""||unionclientbankno==null){
			  mini.alert("收款方银行联行号不能为空！");
			  return false;
		  }
		  var depositprovince=getMiniEditFormField("put_current.depositprovince").getValue();
		  if(depositprovince==""||depositprovince==null){
			  mini.alert("收款方银行开户省不能为空！");
			  return false;
		  }
		  var depositcity=getMiniEditFormField("put_current.depositcity").getValue();
		  if(depositcity==""||depositcity==null){
			  mini.alert("收款方银行开户市不能为空！");
			  return false;
		  }
				return true;
		},
		updateOperCallBack: function(miniTable,rowData){
			...
			if(rowData.settlemethod == "payfund6"){
				for(var i = 0;i < datas.length;i ++){
					if(datas[i].paymentid == rowData.paymentid&&datas[i].settlemethod=="payfund11"){
		...
		removeOperCallBack:function(miniTable, rowDatas){ 
	           var guarantable=mini.get("premise_condition");
		...
		columns: [
			...
			{field: 'devicename', header: '设备名称', visible: false,formEditorConfig:{fieldVisible: false}},
			...
			{field: 'fundtypename', header: '款项节点',formEditorConfig:{readOnly : true,newLine:true}},   
			{field: 'alreadymoney', header: '已付款金额',visible: false,summary : true,dataType : "currency"}, 
			{field: 'oldplanmoney', header: '原计划付款金额',visible: false,summary : true,dataType : "currency",formEditorConfig:{}},
			{field: 'oldplandate', header: '原计划付款时间',visible: false},
			{field: 'factmoney', header: '付款金额',summary : true,dataType : "currency",
				...
				{field: 'settlemethod', header: '结算方式', visible: false,formEditorConfig:{
					...
					onSelect: function($me,queryInput,rowData) {
						mini.get("put_current_editFormPopupWindow_form_billnum").disable(true);
						if("银行承兑汇票"==rowData.name||"商业承兑汇票"==rowData.name){
							mini.get("put_current_editFormPopupWindow_form_billnum").enable(true);
						}
					},
					params: {
					     pid: 'PayFund',
				...
				{field: 'settlemethodname', header: '结算方式',formEditorConfig:{fieldVisible: false,
					...
					fillProperty : 'name'}},
				{field: 'factobjectname', header: '支付对象',width:150,formEditorConfig:{fieldVisible: false}},
				{field: 'factobject', header: '支付对象',visible: false,formEditorConfig:{
					...
					onSelect:function(combo,rowData){
						...
						if(customertype==""){
							getMiniEditFormField("put_current.customertype").setValue("1");
						}else{
						getMiniEditFormField("put_current.customertype").setText(customer);
						getMiniEditFormField("put_current.customertype").setValue(customertype);
						getMiniEditFormField("put_current.customertypename").setValue(customer);}
						clearCustbank();
						},
					params: {
				...
				{field: 'connum', header: '采购合同号',formEditorConfig:{fieldVisible: false}},
				{field: 'contractname', header: '采购合同名称',formEditorConfig:{fieldVisible: false}},
				{field: 'contractmoney', header: '采购合同总额',summary : true,dataType : "currency",formEditorConfig:{fieldVisible: false}},
				{field: 'overcontractmoney', header: '采购合同余额',summary : true,dataType : "currency",formEditorConfig:{fieldVisible: false}},
		...
		{field: 'customertypename', header: '客户类型',visible: true,
			...
			{field: 'factdate', header: '付款日期',formEditorConfig:{newLine:true,type:'date'}},
			{field: 'accountingdate', header: '会计处理日',formEditorConfig:{type:'date',required: true}},
			...
			{field: 'memoname', header: '本方账户别名',visible: false},
			{field: 'memo',header: '本方账户别名',visible: false,formEditorConfig: {
				type : 'queryinput',
				labelWidth:100,
				newLine:true,
				multiSelect : false,
				...
				onSelect:function($me, inputObj, rowData){
					mini.getbyName("account").setValue(rowData.accname);
					mini.getbyName("accnumber").setValue(rowData.accnumber);
					mini.getbyName("accountbank").setValue(rowData.accbank);
					mini.getbyName("memoname").setValue(rowData.memoname);
		...
		{field:'accountbank',header:'本方银行',width:150,formEditorConfig: {readonly:true}},
			...
			//银行行号--
			{field: 'clientbankno', header: '银行行号',width:150,visible:true,
				formEditorConfig:{
					fillFromFieldName : 'clientbank',
...
function initCustbank(){
    var factobj=getMiniEditFormField("put_current.factobjectid");
    var clientbank=getMiniEditFormField("put_current.clientbank");
    fact=factobj.getValue();
