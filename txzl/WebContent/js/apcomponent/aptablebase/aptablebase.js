	...
	var ApTableBase = {
		...
		initColumnsElements : function(renderType, columnsConfig, config, miniTable, isDestroy) {
			...
			for ( var iii = 0; iii < columnsLen; iii++) {
				...
				if(formEditType == 'date' || formEditType == 'combobox'){
				...
				if (formEditType == 'text') {
					...
					if(formEditorConfig["click"]){//增加一个文本框单击事件功能 by zhulh
						otherAttributes += "onclick='"+formEditorConfig["click"]+"'";
					}
				}else if(formEditType == 'thousand'){
					miniClassName = "mini-textbox";
					var oldonkeyup = formEditorConfig.onkeyup;
					if(!oldonkeyup){
						otherAttributes += " vtype='thousand' onkeyup='setapcolumnformatvalue' onblur='setapcolumnformatvalue'";
					}
				...
				htmlArray.push(editFormItemLabelHtml.replace("{labelWidth}", labelWidth).replace("{labelHeader}", labelText).replace("{labelOtherAttributes}", labelOtherAttributes));
				var tdPrefixContent = "";
				...
				if (isDataRange) {
					...
					var editFormItemContentHtml = "<td class='td-input" + tdDisplayClass + "' colspan='" + tdColspan + "'>" + editFormItemInnerHtml + appendHtml + "</td>" + itemHtmlSuffix;
		...
		getQueryAreaParams : function(miniTableId, isClear) {
			...
			for ( var p in params) {
				...
				if (rp.test(p)) {
					...
					var value = params[p].trim().replace(/'/gim,'&#39;').replace(/"/gim,'&quot;').replace(/&#92;/gim,'\\');
					params[key] = value;
					delete params[p];
				}
			}
					...
					mini.confirm(confirmInfo, locale['confirmOperation'], function(buttonText) {
						if ("ok" == buttonText) {
							for(var i = 0 ; i < fields.length ; i++){
								if(fields[i].vtype == 'float' || fields[i].vtype == 'double'){
									fields[i].setValue(fields[i].getValue().replace(/,/g,''));
								}
								if(typeof(fields[i].getValue()) == 'string'){
									fields[i].setValue(fields[i].getValue().replace(/'/gim,'&#39;').replace(/"/gim,'&quot;').replace(/\\/gim,'&#92;'));
								}
							}
				...
				success : function(resultJson) {
					...
					case "SUCCESS": {
						if(config.submitReturnCallBack){
							config.submitReturnCallBack(miniTable, editFormItemOperFlag, operText, resultJson)
						}else{
							miniTable.reload();
							mini.unmask(document.body);
		...
		exportExcel : function(miniTable, columnsConfig, config) {
           ...
           var loadmode=config.loadMode||"local";
			...
			params['excelTitleName'] = config.title || config.exportTitle || "export";
			...
			for ( var p in params) {
				...
				tempStr = typeof tempStr == "undefined" ? "" : tempStr;
		...
		importExcel: function(miniTable, columnsConfig, config) {
			...
			uploadAttachmentFileWindow_html += '<div id="id_minitableimport" class="mini-window"  closed="true" showModal="true" title="导入Excel('+(typeof(config.title)=="undefined"?"":config.title)+')" style="display:none;width:400px;text-align:center;background-color:#FDF9F8;">';
			...
			uploadAttachmentFileWindow_html += "<input type='hidden' name='jscallback' value='" + (config.jscallback||"importMiniTableCallBack") + "'/>";
			...
			//天信项目 :针对项目编号做对应
			uploadAttachmentFileWindow_html += "<input type='hidden' name='importprojDevelop' value='" + (config.importprojDevelop||"") + "'/>";
			//天信项目：资金预实明细表针对日期做对应
			uploadAttachmentFileWindow_html += "<input type='hidden' name='importDateid' value='" + (config.importDateid||"") + "'/>";
			//天信项目：银行台账ID做对应
			uploadAttachmentFileWindow_html += "<input type='hidden' name='importLoanid' value='" + (config.importLoanid||"") + "'/>";
			//天信项目：银行台账ID做对应
			uploadAttachmentFileWindow_html += "<input type='hidden' name='importLoanaccountid' value='" + (config.importLoanaccountid||"") + "'/>";
			//天信项目：与loan_rent_planID做对应
			uploadAttachmentFileWindow_html += "<input type='hidden' name='importPlanid' value='" + (config.importPlanid||"") + "'/>";
			try{
			 uploadAttachmentFileWindow_html += "<input type='hidden' name='sgrade_id' value='" + (flowUnid||"") + "'/>";	 
			 }catch(e){
			 }
			...
			$("#id_minitableimport_save").bind('click',function(){
				...
				if(filename==""){mini.alert("请选择上传的文件");return false;}
