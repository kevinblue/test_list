...
function createAttachmentFileContainers() {
	...
	if (uploadWindow == undefined) {
		...
		uploadAttachmentFileWindow_html += '<div id="id_uploadAttachmentFileWindow" title="附件" class="mini-window" style="display:none;width:600px;height:500px;background-color:#FDF9F8;">';
		...
		uploadAttachmentFileWindow_html += '			<input type="hidden" name="docid" value="custid">';
		...
		uploadAttachmentFileWindow_html += '				<tr><td class="input_label_desc">' + tracywindyLocale['uploadCategory'] + ':</td><td class="input_value"><label id="id_uploadAttachmentFileWhich"/></label></td></tr>';
		uploadAttachmentFileWindow_html += '				<tr><td class="input_label_desc">' + tracywindyLocale['currentTime'] + ':</td><td class="input_value"><label id="id_uploadAttachmentFileTime"/></label></td></tr>';
		uploadAttachmentFileWindow_html += '				<tr><td class="input_label_desc">' + tracywindyLocale['selectFile'] + ':</td><td class="input_value"><input type="button" value="' + tracywindyLocale['addButtonText'] + '" onclick="addUploadLocation(this)" locationindex="0" /></td></tr>';
		...
		uploadAttachmentFileWindow_html += '				<tr><td class="input_label_desc">' + tracywindyLocale['fileDescription'] + ':</td><td class="input_value"><textarea name="remark" id="id_uploadAttachmentFileRemark"/></textarea></td></tr>';
		uploadAttachmentFileWindow_html += '				<tr><td class="input_label_desc" colspan="2" style="color:red;">' + tracywindyLocale['uploadAlert3'] + '</td></tr>';
		...
		uploadAttachmentFileWindow_html += '						<a class="mini-button" onclick="__uploadActionSubmit"><span>' + tracywindyLocale['confirmButtonText'] + '</span></a>';
		uploadAttachmentFileWindow_html += '						<a class="mini-button" onclick="__uploadActionCancel"><span>' + tracywindyLocale['cancelButtonText'] + '</span></a>';
...
//预览
function showFile(filedict,filename){
...
function batchUploadFiles(table, batchDownloadAttachmentFileTitle) {
	...
	if (len < 1) {
		mini.alert(tracywindyLocale['downloadAlert1']);
		return;
	} else {
		for ( var i = 0; i < len; i++) {
...
var showUploadFilesList = function(e) {
	...
	if(!fileListUUIDStr){
		...
		for ( var i = 0; i < fileUUIDListArr.length; i++) {
			...
			var fileNameTemp = fileNameListArr[i].length > 50 ? fileNameListArr[i].substring(0, 50)+"..." : fileNameListArr[i].length;
			...
			tempValue +="<a href='javascript:void(0)' onclick='showFile(\""+fileUUIDListArr[i]+"\",\""+ fileNameListArr[i]+"\")'>"+"预览</a> &nbsp;&nbsp;&nbsp;&nbsp;"+ "<a style='text-decoration:none;' onclick='downloadUploadFile(\"" + fileUUIDListArr[i] + "\");' title='" + fileNameListArr[i] + "' href='javascript:void(0);'>" + fileNameListArr[i] + "</a></td>";
			tempValue += "<td style='border:0px solid #FFF;'>【" + fileTypeListArr[i] + "】</td>";
			tempValue += "<td style='border:0px solid #FFF;'>【" + fileUploadUserListArr[i] + "】</td>";
			tempValue += "<td style='border:0px solid #FFF;'>【" + fileSize + "&nbsp;" + fileSizeFlag + "】</td>";
		
			if (!e.sender.isReadOnlyMini) {
//				if (window.currentTaskActivityDetailId && (window.currentTaskActivityDetailId == fileAdListArr[i])) {
					tempValue += "<td style='border:0px solid #FFF;'><a onclick='removeUploadFile(\"" + e.sender.id + "\",\"" + fileUUIDListArr[i] + "\");' style='position:relative;top:-2px' title='" + tracywindyLocale['removeButtonText'] + "' href='javascript:void(0);'><img border=0 align='absmiddle' width=14  src='" + getRootPath() + "/images/icon_delete.gif'/></a></td>";
//				}
			}
...
// unionkey
var newAttachmentFileUpload = function(config) {
	...
	var htmlTitle = isEnglish()? 'Batch Download Files': '批量下载文件';
	var filename=isEnglish() ? "File Name" :"文件名称";
	var operOne=isEnglish()? "Oper One":"上载";
	var operTwo=isEnglish()?"Oper Two":"上载文件清单";
	var remarks=isEnglish()?"Remarks":"备注";
	var tableConfig = {
		multiSelect: true,
		...
		columns: [
			...
			{type: 'indexcolumn'},
			{type: 'checkcolumn'},
			{field : 'attachmentFileDictId', header : '文件编号', visible : false},
			{field : 'attachmentFileDictFileName', header : filename, width : '27%',
				renderer: function(e){
					var data = e.record;
					return isEnglish() ? data.attachmentFileDictFileNameEn : data.attachmentFileDictFileName;
				}
			},
			...
			{field : 'operOne', header : operOne, align : 'center', width : '8%',
				renderer : function(e) {
					if (!identifierOne) {
						return "<font color='red'>关键字为空</br>不能上载文件</font>";
					} else {
						var test="<a href='javascript:void(0);' onclick='addUploadFile(\"id_customworkflowattachment\"," + e.rowIndex + ",\"add\");'>" + tracywindyLocale['uploadAttachments'] + "</a>";
						return  test;
					}
				}
			},
			{field : 'operTwo', header : operTwo ,width: '55%',
				renderer: function(e){
					return showUploadFilesList(e);
				}
			}
			,
			{field : 'remark', header :remarks,width: '10%',
				renderer: function(e){
...
function removeUploadFile(tableId, uploadAttachmentFileDetailId) {
	mini.confirm(tracywindyLocale['removeAlert1'] + "？", tracywindyLocale['removeButtonText'] +"？", function(action) {
		if (action == 'ok') {
			/*mini.mask({
				el : document.body,
				cls : 'mini-mask-loading',
