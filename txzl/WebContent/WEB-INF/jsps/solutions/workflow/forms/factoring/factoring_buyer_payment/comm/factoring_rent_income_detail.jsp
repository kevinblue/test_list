...
jQuery(function(){
	...
	tenwa.createTable({
		...
		columns: [
			...
			{field: 'rent', header: '回笼债权总计',type:'double',summary : true,dataType : "currency",formEditorConfig:{newLine:true,vtype:'float',readOnly:true}},
			{field: 'corpus', header: '回笼应收账款',type:'double',summary : true,dataType : "currency",formEditorConfig:{vtype:'float',onkeyup: 'checkHire(e,"corpus")'}},
			{field: 'interest', header: '回笼保理收入',type:'double',summary : true,dataType : "currency",formEditorConfig:{newLine:true,vtype:'float',onkeyup: 'checkHire(e,"interest")'}},
			...
			{field: 'rentadjust', header: '债权调整',type:'double',summary : true,dataType : "currency",formEditorConfig:{readOnly:true,newLine:true,vtype:'float'}},
			{field: 'corpusadjust', header: '应收账款调整',type:'double',summary : true,dataType : "currency",visible: false,formEditorConfig:{vtype:'float'}},
			{field: 'interestadjust', header: '保理收入调整',type:'double',summary : true,dataType : "currency",formEditorConfig:{vtype:'float',onkeyup:'checkHire(e,"interestadjust")'}},
			{field: 'penaltyadjust', header: '罚息调整',type:'double',summary : true,dataType : "currency",visible: false,formEditorConfig:{vtype:'float'}},
			{field: 'curcorpusoverage', header: '应收账款余额',visible:false,formEditorConfig:{vtype:'float'}},
			{field: 'curinterestoverage', header: '保理收入余额',visible:false,formEditorConfig:{vtype:'float'}},
...
function checkHire(e,type){
	...
	if(type == "corpus"){
		if(compareTwoField( $("input[name=corpus]").val(),curcorpusoverage.getValue())){
            mini.alert("本次回笼应收账款金额大于应收账款余额"+curcorpusoverage.getValue());
	...
	if(type == "interest"||type=="interestadjust"){
		 ...
		 if(compareTwoField(sum,curinterestoverage.getValue())){
	            mini.alert("本次回笼保理收入加上保理收入调整的金额大于保理收入余额"+curinterestoverage.getValue());
