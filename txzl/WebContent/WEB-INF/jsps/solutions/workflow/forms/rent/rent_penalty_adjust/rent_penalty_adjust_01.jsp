...
function workflowSubmitCallBack(buttonText)//返回true提交，返回false不提交 ，该回调函数可与用表单字段验证
{
	...
	if(mini.get("rent_penalty_adjust_detail")!=null){
		...
		//alert(allpenalty);
		mini.get("allpenalty").setValue(allpenalty);
	}else{
		var allpenalty=0;
		mini.get("allpenalty").setValue(allpenalty);
	}
	...
	miniui_ext.saveIncludeData("tabDetails");
	var cflag=checkFundRentIncome("rent_penalty_adjust_detail");//合数据库进行比较
	if(!cflag){return false;}
	if(mini.get("rent_penalty_adjust_detail").getData().length<=0){mini.alert("请填写本次要减免的租金计划");}
	return true;
}
//提交路由，选择下一步的节点
function workflowNextRouteCallBack(buttonText,requestNextRoute){
	if(buttonText == "Submit"){
		...
		allpenalty = Number(allpenalty.replace(",",""));
		if(allpenalty>=200){
			requestNextRoute.value= '200及以上';
		}else {
			requestNextRoute.value= '200以下';
		}
	}
}
