			...
			<c:choose>
				<c:when  test="${requestScope['cust_info.custscale'] =='小型企业'}">
					<div title="小型企业评分" iconCls="icon-cut"> 
						<jsp:include page="common/small_business.jsp"></jsp:include>
					</div>  
				</c:when>  
				<c:when  test="${requestScope['cust_info.custscale'] =='大中型企业'}">
					<div title="大中型企业评分" iconCls="icon-cut"> 
						<jsp:include page="common/big_business.jsp"></jsp:include>
					</div> 
				</c:when>
				<c:when  test="${requestScope['cust_info.custscale'] =='供应商'}">
					<div title="供应商评级" iconCls="icon-cut"> 
					<input id="Score"  class="mini-textbox"  name="Score"   style="display:none;"
				value='${empty Score ? "":Score}'>
						<jsp:include page="common/supplier.jsp"></jsp:include>
					</div> 
				</c:when>
			</c:choose>   
