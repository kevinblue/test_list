...
jQuery(function(){
	...
	var tools= [{
		...
		plain : true,
		iconCls : 'icon-add',
		handler : function(miniTable, buttonText) {uploadFileMeeting();}
	},'-','remove'];
	...
	tenwa.createTable({
		id: "proj_meeting_minutes_report_list",
		renderTo: "id_table_proj_meeting_minutes_report_list",
		width: '100%',
		height: 200,
		lazyLoad: false,
		title: "",
...
function uploadFileMeeting() {
	var uploadutil = new uploadUtil({
			...
			jscallback : 'reloadcustcontactfileMeeting'
		});
	uploadutil.createFileAndShow();
}
function reloadcustcontactfileMeeting(message){
	mini.alert(message);
	mini.get("id_uploadfile").hide();
	mini.unmask(document.body);
	mini.get("proj_meeting_minutes_report_list").reload();
}
</script>
<div id="id_table_proj_meeting_minutes_report_list"></div>