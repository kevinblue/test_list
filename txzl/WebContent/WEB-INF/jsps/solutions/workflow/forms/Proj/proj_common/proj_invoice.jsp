...
jQuery(function(){
	...
	/* 		importOrExPortParam:{'exclid':'EXCLcheck'},//用来校验租赁物单价*数量=总价
			...
			data: JsonUtil.decode('${empty json_proj_invoice_str ? "[]" : json_proj_invoice_str }'),
			validateForm:function(miniTable, miniForm, editFormItemOperFlag, addIndex){
				var startlist = mini.getbyName('invoicedate').getValue();
				var endlist = mini.getbyName('invoiceexpiredate').getValue();
				if(Number(endlist) < startlist){
					mini.alert('发票到期日不能小于开始期次！！！');
					return false;
				}
				return true; 
			},	
...
<div id="id_table_proj_invoice_detail"></div>