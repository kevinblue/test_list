...
jQuery(function(){
	...
	var tools= [{
		...
		plain : true,
		iconCls : 'icon-add',
		handler : function(miniTable, buttonText) {uploadFileCrew();}
	},'-','remove'];
	...
	tenwa.createTable({
		id: "selection_accessories_report_list",
		renderTo: "id_table_selection_accessories_report_list",
		width: '100%',
		height: 200,
		lazyLoad: false,
		title: "",
...
function uploadFileCrew() {
	var uploadutil = new uploadUtil({
			...
			jscallback : 'reloadcustcontactfileCrew'
		});
	uploadutil.createFileAndShow();
}
function reloadcustcontactfileCrew(message){
	mini.alert(message);
	mini.get("id_uploadfile").hide();
	mini.unmask(document.body);
	mini.get("selection_accessories_report_list").reload();
}
</script>
<div id="id_table_selection_accessories_report_list"></div>