...
<script type="text/javascript">
		...
		function irrCalculateSuccess( message){
			...
			if(result.issucess == 'true'){
            	...
            	$mini("firstplandate").setValue(result.firstplandate);//一期
            	$mini("secondplandate").setValue(result.secondplandate);//二期
            	...
            	if(mini.get('rentorrate').getValue() != 'rate'){
            	...
            	//result.fundrentplan 是对象、 不是字符串
            	//如果需要查看字符串、则用 mini.encode(fundrentplan)
            	//租金计划  
            	//现金流 
            	//资金收付计划
            	mini.get("fund_rent_plan_frame").isinitData=1;
            	mini.get("fund_cash_plan_frame").isinitData=1;
            	mini.get("fund_fund_plan").isinitData=1;
            	
            	var fundrentplan =mini.decode(result.rentplanlist) ;
            	var finacashdetail =mini.decode(result.cashdetaillist) ;
            	var fundchargeplan = mini.decode( result.fundchargeplan);
            	...
            	jQuery("#id_json_fund_fund_charge_str").val(mini.encode(fundchargeplan));
            	updateInputThousand();
            	mini.alert("测算成功！", "提示", function(){
            		//$miniSetCombValue('incomenumberyear','income_0','不等期');
