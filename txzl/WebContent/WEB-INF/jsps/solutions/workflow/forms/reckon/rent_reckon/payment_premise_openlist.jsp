...
<script type="text/javascript">
	...
	if('${param.isView}' == 'true'){showTools = false};
  						...
  						handler : function(miniTable, buttonText) {
  							...
  							if (rows.length > 0) {
  								...
  								for (var i = 0; i < paymentjson.length; i++) {//资金收付计划传递过来的JSON数组
  									var temp = 0;
  									var numObj = {};
  									var strfundtypename=paymentjson[i].fundtypename
  									for (var j = 0; j < rows.length; j++) {//弹出窗口中的结果集
  										if(strfundtypename==rows[j].paymentcategory){
  											var fundplan = {};
  	  										fundplan.docid = paymentjson[i].docid;
  	  										fundplan.projnumber = paymentjson[i].contractid==""?window.parent.mini.get("contract_id").getValue():paymentjson[i].contractid;  
  	  										fundplan.feetype = paymentjson[i].fundtype;
  	  										fundplan.feetypename =strfundtypename;//paymentjson[i].feetypename;feetypename中文乱码，天信需求显示fundname
  	  										fundplan.paymentid = "第" + paymentjson[i].paymentid+ "笔";
  	  										fundplan.conditionname = rows[j].conditionname;
  	  										fundplan.devicenamename = paymentjson[i].devicenamename;
  	  										fundplan.devicename = paymentjson[i].devicename;
						...
						{type:'checkcolumn'},
  						...
  						{field:'conditionname',header:'条件名称',  								
  						 ...
  						 {field:'contracttype',header:'合同类型',
  			  			...
  			  			{field:'paymentcategory',header:'款项节点',
	  			  				    	headerAlign:'center',
	  			  				    	width:150,
	  			  				    	allowSort:true, 
	  			  				    	formEditorConfig:{
  		...
  		function getPayNum(paymentjson, checkparentdata, numObj){
  			var num = 1;
  			var keyTemp =paymentjson.devicename+"_"+paymentjson.fundtype + "_" + paymentjson.paymentid;
  			if(numObj.keyTemp){
  				...
  				for(var i = 0;i < checkparentdata.length; i++){
  					...
  					if(paymentjson.devicename==checkparentdata[i].devicename&&paymentjson.fundtype == checkparentdata[i].feetype
