...
jQuery(function(){
	...
	seajs.use([ "js/apcomponent/aptable/aptable.js" ], function(ApTable) {
		new ApTable({
						...
						handler : function(miniTable, buttonText) {
							var params={};
						     ...
						     for(var i=0;i<cdates.length;i++){
				                    if(cdates[i].filename=="财产一切险投保单.docx"){
				                    	mini.alert("您已经生成财产一切险投保单！");
				                        return false;
				                    }
							    }	
							var fileTeplate=new fileTemplateConfig({
								templateno : 'F-201603003',
			        ...
			        },'-',
			        {
						...
						handler : function(miniTable, buttonText) {
							var params={};
						     ...
						     var cdates=miniTable.getData();
						     for(var i=0;i<cdates.length;i++){
				                    if(cdates[i].filename=="机器损坏险投保单.docx"){
				                   	   mini.alert("您已经生成机器损坏险投保单！");
				                       return false;
				                    }
							    }	
							var fileTeplate=new fileTemplateConfig({
								templateno : 'F-201603004',
								tableid : 'credit_report',
								modelname : '财产一切险投保单',
								returntype : 'listtocurpage',
								parames : params
			...
			data: JsonUtil.decode('${empty id_json_credit_report_str ? "[]" : id_json_credit_report_str }'),
			updateOperCallBack: function(miniTable,formData){
			},
			operValidate:function(miniTable, rowDatas,flag){
// 				  if(flag=="remove"){
// 				   for(j=0;j<rowDatas.length;j++){
// 					   if(rowDatas[j].filename!="客户尽调报告.docx"){
// 	                  	 mini.alert("您只能删除客户尽调报告！");
// 	                        return false;
// 	                   }
// 				   }}
			       return true;
