...
<script type="text/javascript">
	...
	window["func_chart_${id}"] = function(){
		...
		jQuery('#chart_data_${id}').width(pageWidth);
...
<div id='chart_${id}' style="border:1px solid #ddd;" class="x-panel-table-div x-panel-table-div-border">
	...
	<r:if test="${fn:length(searchs) > 0}">
		<div id='chart_search_${id}' style="display:none;overflow: hidden;" class="mini-window"  showCloseButton="false" showModal="true" allowDrag="true" width="820px" heigth="500px" title="预查询">
				<center style="margin-bottom:20px;width: 810px;overflow: hidden;">
					...
					<div id="div_chart_search_button_${id}" style="padding-top:1px;;text-align: center;overflow: hidden;">
						<a href="javascript:void(0);" class="btn btn-primary" id="queryOnlick" onclick="doChartSearch(queryObject_${id});"><span>查询</span></a>
						<a href="javascript:void(0);" class="btn btn-primary" onclick="doChartSearch(queryObject_${id},'filter');"><span>清空</span></a>
					</div>
				</center>
		</div>
	</r:if>
							...
							<r:if test="${status.last}">
								...
								<r:if test="${((status.index+1)%columnCount eq 0) }">
									...
									<td colspan="${(columnCount - (status.index+1)%columnCount) * 2}">
										<div id="chart_div_filter_button_${id}" style="float:left;padding-top:1px;margin-left:10px;">
											<a href="javascript:void(0);" class="btn btn-primary" id="queryOnlick" onclick="doChartSearch(queryObject_${id});"><span><spring:message code="Search" text="查询"/></span></a>
											<a href="javascript:void(0);" class="btn btn-primary" onclick="doChartClear(queryObject_${id},'filter');"><span><spring:message code="Clear" text="清空"/></span></a>
										</div>
									</td>
								</tr>
							</r:if>
	...
	<div id="chart_data_${id}" style="overflow:auto;"></div>
</div>
<r:if test="${empty needjs || (needjs eq '1')}">
