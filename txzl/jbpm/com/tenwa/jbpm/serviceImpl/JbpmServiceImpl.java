...
import com.tenwa.jbpm.utils.MessagePushUtil;
	...
	*/
	...
	public void updateWorkflowConfigSynchronized(String type,String host, String port, String dbname, String user,String password,String updateWorkflowIds,Locale locale) throws Exception
	{
		...
		if(null != updateWorkflowIds){
			designerSql +=" where t_jbpm_designer_transfer.id_ in("+StringUtil.toSqlJoinString(updateWorkflowIds,",")+") ";
		}
		List<Map<String,String>> workflowList = fromJdbcTemplate.query(designerSql, new RowMapper<Map<String,String>>(){
			@Override
		...
		for(Map<String,String> transferMap : workflowList ){
			...
			if(null != fromDBDeployId){
				String detailSql    = "select * from t_jbpm_activity_detail where jbpm4_deployment_dbid_ ='"+fromDBDeployId+"'";
				activitiesDetailDatas = fromJdbcTemplate.query(detailSql, new RowMapper<Map<String,String>>(){
					@Override
					public Map<String, String> mapRow(ResultSet rs, int index)throws SQLException 
				    {
						Map<String,String> rowDataMap = new HashMap<String,String>();
	...
	public String updateRequestProcessTaskForm(HttpServletRequest request,String currentTaskId,Map<String,String> modelData) throws Exception 
	{
		...
		//pending delegate signature read add by zhangc 2015-12-16 增加代办以及已办和流程实例的权限控制
		User user = SecurityUtil.getPrincipal();
		String currentRequestTaskType = modelData.get("currentRequestTaskType");
		if(null != modelData.get("jbpmWorkflowHistoryInfoUserId")){
			if(currentRequestTaskType != null){
				JBPMUtil.checkCurrentActivityNowUserPower(modelData.get("jbpmWorkflowHistoryInfoUserId"), currentRequestTaskType, user, this, jhi_completedTaskImpl);
			}else{
				throw new TenwaJbpmException("无法判断当前任务的类型");
			} 
		}else{
			String flag = modelData.get("flag");
			String designId = jhi_completedTaskImpl.getDeploymentImpl().getJbpmWorkflowDesigner().getId();
			JBPMUtil.checkCurrentUserHasViewPower(flag, designId, this, jhi_completedTaskImpl, user);
		}
	...
	public void getDynamicRequestInitiatorRoute(HttpServletRequest request,String conditionRouteType,String conditionRouteValue,Map<String,String> modelData) throws Exception{
		...
		if(null == request.getAttribute("dynamicRequestInitiatorRoute")){
	...
	{
		JBPMWorkflowHistoryInfo jhi_completedTaskImpl = (JBPMWorkflowHistoryInfo)this.jbpmDao.findEntityByID(JBPMWorkflowHistoryInfo.class, jhi_completedTaskImplId);
		String jbpmWorkflowHistoryInfoUserId = request.getParameter("jbpmWorkflowHistoryInfoUserId");
		Set<JBPMWorkflowHistoryInfoUser> jbpmUsers = jhi_completedTaskImpl.getJbpmWorkflowHistoryInfoUsers();
		if(!isLastBack){
			Boolean isCurrentTaskUser = false;
			for(JBPMWorkflowHistoryInfoUser jbpmUser : jbpmUsers){
				if(jbpmUser.getId().equals(jbpmWorkflowHistoryInfoUserId)){
					isCurrentTaskUser = true;
					break;
				}
			}
			if(!isCurrentTaskUser){
				throw new TenwaJbpmException("当前代办与当前处理人信息不匹配，请核对后提交！！！");
			}else{
				JBPMUtil.checkCurrentActivityNowUserPower(jbpmWorkflowHistoryInfoUserId, "pending", SecurityUtil.getPrincipal(), this, jhi_completedTaskImpl);
			}
		}
					...
					if((av.getType().equalsIgnoreCase(ad.getActivityType()))&&(av.getName().equals(ad.getActivityKey())))
					{
						...
						if(!"start".equalsIgnoreCase(ad.getActivityType())){
							...
							if(!"end".equalsIgnoreCase(ad.getActivityType())){
								try{
									//add by zhanc
									if(ad.getActorType() != null && ad.getActorType().equalsIgnoreCase("submitDefinedRelation")){
									...
									{
										String hql = "select jiu.planActor from JBPMWorkflowHistoryInfoUser jiu where jiu.jbpmWorkflowHistoryInfo.historyProcessInstanceImpl = ? and jiu.jbpmWorkflowHistoryInfo.activityDetail = ?";
										dealUsers = this.jbpmDao.findResultsByHSQL(hql, historyProcessInstanceImpl,ad);
									}else{
										e.printStackTrace();
										continue;
									}
	...
	public List<JBPMWorkflowHistoryInfo> getProcessedHistoryLogInfoList(String historyProcessInstanceImplDbid,String currentJbpmWorkflowHistoryInfoId) throws Exception{
		List<Object> sourceProcessedHistoryLogInfoList = this.jbpmDao.getSinglObjectListByHSQL("select new map(jwhi.id as id,jwhi.historyTaskInstanceImpl as historyTaskInstanceImpl,jwhi.historyProcessInstanceImpl as historyProcessInstanceImpl,jwhi.deploymentImpl as deploymentImpl,jwhi.keyOne as keyOne,jwhi.keyTwo as keyTwo,jwhi.workflowName as workflowName,jwhi.workflowDisplayName as workflowDisplayName,jwhi.processInstanceState as processInstanceState,jwhi.processDefinitionId as processDefinitionId,jwhi.requestInitiatorUser as requestInitiatorUser,jwhi.requestInitiator as requestInitiator,jwhi.requestInitiatorRealName as requestInitiatorRealName,jwhi.activityDetail as activityDetail,jwhi.fromButtonTextToCome as fromButtonTextToCome,jwhi.historyActivityName as historyActivityName) from  JBPMWorkflowHistoryInfo jwhi where jwhi.historyProcessInstanceImpl.dbid = ? and jwhi.historyTaskInstanceImpl is not null order by jwhi.historyTaskInstanceImpl.endTime desc,jwhi.historyTaskInstanceImpl.startTime desc",Long.parseLong(historyProcessInstanceImplDbid));
		List<JBPMWorkflowHistoryInfo> works = new ArrayList<JBPMWorkflowHistoryInfo>();
		for(Object sourceProcessedHistoryLogInfo : sourceProcessedHistoryLogInfoList){
			Map map = (Map)sourceProcessedHistoryLogInfo;
			BeanInfo beanInfo = Introspector.getBeanInfo(JBPMWorkflowHistoryInfo.class);
			PropertyDescriptor[] properties = beanInfo.getPropertyDescriptors();
			JBPMWorkflowHistoryInfo returnInfo = new JBPMWorkflowHistoryInfo(); 
			for(PropertyDescriptor property : properties){
				String tempFildName = property.getName();
				if(map .containsKey(tempFildName)){
					try{
						//System.out.println(tempFildName);
						Method method = property.getWriteMethod();
						method.invoke(returnInfo,map.get(tempFildName)); 
					}catch(Exception e){
			...
			List<JBPMWorkflowHistoryInfoUser> jbpmWorkflowHistoryInfoUsers = this.jbpmDao.findResultsByHSQL("from JBPMWorkflowHistoryInfoUser jwi where jwi.jbpmWorkflowHistoryInfo.id=?", returnInfo.getId());
			List<JBPMWorkflowSignatureUser> jbpmWorkflowSignatureUsers = this.jbpmDao.findResultsByHSQL("from JBPMWorkflowSignatureUser jwi where jwi.jbpmWorkflowHistoryInfo.id=?", returnInfo.getId());
			List<JBPMWorkflowReadUser> jbpmWorkflowReadUsers = this.jbpmDao.findResultsByHSQL("from JBPMWorkflowReadUser jwi where jwi.jbpmWorkflowHistoryInfo.id=?", returnInfo.getId());
			returnInfo.setJbpmWorkflowHistoryInfoUsers(new HashSet<JBPMWorkflowHistoryInfoUser>(jbpmWorkflowHistoryInfoUsers));
			returnInfo.setJbpmWorkflowReadUsers(new HashSet<JBPMWorkflowReadUser>(jbpmWorkflowReadUsers));
			returnInfo.setJbpmWorkflowSignatureUsers(new HashSet<JBPMWorkflowSignatureUser>(jbpmWorkflowSignatureUsers));
			works.add(returnInfo);
		}
		...
		for(int si=0;si<works.size();si++)
		{
			JBPMWorkflowHistoryInfo obj = works.get(si);
			//判断该代办是不是当前流程代办的退回代办
	...
	public void getRequestFormDealedInfo(HttpServletRequest request,JBPMWorkflowHistoryInfo jhi_completedTaskImpl,JBPMWorkflowHistoryInfo jhi_processInstance,Map<String,String> modelData,Set<String> activityNames) throws Exception
	{
		...
		if(jbpmWorkflowHistoryInfoUser != null){
			jbpmWorkflowHistoryInfoUser.setIsReaded(true);
		}
	...
	public void addReadPersonToTask(JBPMWorkflowHistoryInfo jhi_completedTaskImpl,ActivityDetail assignToTaskAd,Map<String,String> modelData,String jbpmWorkflowHistoryInfoUserId,User readActor,String userIds,String splitStr,String readAdvise,List<User> userList) throws Exception
	{
	   ...
	   if(null != userList)
	   {
		   for(User u : userList)
		   {
			   ...
			   if(!isExist)
			   {
				   ...
				   //自动传阅发送消息提醒
				   this.sendMessageNoticeToUser(jhi_completedTaskImpl, assignToTaskAd, readedActor, modelData, SubmitTypeEnum.READ);
			   }
		   }
	   }
	   ...
	   for(String userId : userIdsList)
	   {
		   ...
		   if(!isExist)
		   {
			   ...
			   addWorkFlowAppPush(readUser.getId());
			   //继续传阅发送消息提醒
			   this.sendMessageNoticeToUser(jhi_completedTaskImpl, jhi_completedTaskImpl.getActivityDetail(), readedActor, modelData, SubmitTypeEnum.READ);
		   }
	   }
	}
	@Override
		   ...
		   if(null != userList)
		   {
			   for(User u:userList)
			   {
				   ...
				   if(!isExist)
				   {
					   ...
					   //会签发送消息提醒
					   this.sendMessageNoticeToUser(jhi_completedTaskImpl, jhi_completedTaskImpl.getActivityDetail(), signaturedActor, modelData, SubmitTypeEnum.SIGNATURE);
				   }
			   }
			   return;
		   }
	...
	public void addWorkAssignmentPersonToTask(JBPMWorkflowHistoryInfo jhi_completedTaskImpl,String jbpmWorkflowHistoryInfoUserId,String assignmentUserId,String processedAdvise,String processedResult) throws Exception
	{
		...
		if((null !=processedResult) &&(!processedResult.isEmpty()))
		{
		 	jbpmWorkflowHistoryInfoUser.setProcessedResult(new DictionaryData(processedResult));
	 	}else{
	 		jbpmWorkflowHistoryInfoUser.setProcessedResult(null);
	 	}
	...
	public String submitProcessedTaskForm(HttpServletRequest request,String currentTaskId, Map<String,String> modelData) throws Exception 
	{
		...
		else if(WorkflowSubmitTextEnum.assignmentButtonDisplayText.getText().equals(currentTaskSubmitButtonText))
		{
			...
			String processedAdvise = modelData.get("processedAdvise");
			String processedResult = modelData.get("processedResult");
			this.addWorkAssignmentPersonToTask(jhi_completedTaskImpl_inner, jbpmWorkflowHistoryInfoUserId, assignmentedUserId, processedAdvise,processedResult);
			return saveActionReturnResult;
		}
...
//	     String processedAdvise = URLDecoder.decode(request.getParameter("processedAdvise") == null ? "" : request.getParameter("processedAdvise"), "UTF-8");
		 log.info("没有经过转码=========================================" +request.getParameter("processedAdvise") == null ? "" : request.getParameter("processedAdvise"));
		 String processedAdvise = modelData.get("processedAdvise");
		 String processedResult = modelData.get("processedResult");
		 log.info("经过转码过的==========================================="+processedAdvise);
						...
						if(activityName_.equals(activityKey))
						{
							 ...
							 if(isNew)
							 {
								...
								// String actorValue = activityDetail.getActorValue();
								 ...
								 //moditify by zhangc 修改自动提交按钮的权限限制
								 ...
								 //{
									 ...
									 else
									 {
										 ...
										 if(null != changeRequestInitiator){
								 ...
								 for(User u : actorList)
								 {
									 ...
									 this.sendMessageNoticeToUser(jhi_task, pendingActivityDetail, u, modelData, SubmitTypeEnum.SUBMIT);
									 MessagePushUtil.sendMessageToApp(jhi_task, u);
...
//									 }
	...
	public void   sendMessageNoticeToUser(JBPMWorkflowHistoryInfo jwi,ActivityDetail pendingActivityDetail,User u,Map<String,String> modelData,SubmitTypeEnum ste) throws Exception{
		//待办提醒
		String workflowDisplayName = jwi.getWorkflowDisplayName();
		 String messageTypes   = StringUtil.nullToString(pendingActivityDetail.getMessageTypes());
		 if(!messageTypes.isEmpty()){
			 String messageContent = "";
			 String mailSubject  = "[{0}("+DateUtil.getSystemDateTime()+")]"+workflowDisplayName+"->"+pendingActivityDetail.getActivityName();
			 if(jwi.getKeyOne() != null){
				 mailSubject += "->" + jwi.getKeyOne();
			 }
			 if(jwi.getKeyTwo() != null){
				 mailSubject += "->" + jwi.getKeyTwo();
			 }
			 switch(ste){
			    case SUBMIT:{
			    	mailSubject = mailSubject.replace("{0}","代办任务");
			    ...
			    case READ:{
			    	...
			    	messageContent+=workflowDisplayName;
			    	mailSubject = mailSubject.replace("{0}","传阅任务");
			    	break;
			    }
			    case SIGNATURE:{
			    	...
			    	messageContent+=workflowDisplayName;
			    	mailSubject = mailSubject.replaceAll("{0}","会签任务");
			    	break;
			    }
			 }
			 for(String messageType : messageTypes.split(",")){
				 switch(MessageTypeEnum.valueOf(messageType.toUpperCase())){
				   case SMS:{
					   ...
					   if(receiverMobilePhone!= null && receiverMobilePhone.length() > 0 ){
							...
							//SendMessageUtil.sendMessageByMQ(receiverMobilePhone, messageContent);
							String result = SendMessageUtil.sendMessage(receiverMobilePhone, messageContent);
...
//		}
	...
	public void submitAutoProcessedTaskForm(
	 ...
	 ) throws Exception 
	 {
		...
		modelData.put("processedResult", processedResult);
		//提交节点
