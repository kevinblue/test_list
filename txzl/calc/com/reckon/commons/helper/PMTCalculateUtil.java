...
public class PMTCalculateUtil {
	...
	public static BigDecimal getPMT(BigDecimal rate, int nper, BigDecimal pv, BigDecimal fv, int type) {
		...
		return result.setScale(Scale.RENT_SCALE, BigDecimal.ROUND_UP);
	}
	
	
	public static BigDecimal getPMT(BigDecimal rate, int nper, BigDecimal pv, BigDecimal fv, int type, List<BigDecimal> adjust) {
		fv = (fv == null ? BigDecimal.ZERO : fv);
		BigDecimal one = BigDecimal.ONE;
		BigDecimal reciprocal = one.divide(rate.add(one), 20, BigDecimal.ROUND_HALF_UP);
		BigDecimal result = pv.subtract(fv.multiply(reciprocal.pow(nper)));
		
		BigDecimal div = BigDecimal.ZERO;
		for (int i = 0; i < nper; i++) {
			BigDecimal rentTemp = (i < adjust.size() ? adjust.get(i) : null);
			if(rentTemp != null){
				result = result.subtract(rentTemp.multiply(reciprocal.pow(i + (type == 0 ? 0 : 1))));
			} else {
				div = div.add(reciprocal.pow(i + (type == 0 ? 0 : 1)));
			}
		}
		return result.divide(div, Scale.RENT_SCALE, BigDecimal.ROUND_UP);
	}
	
	public static BigDecimal getPMT(BigDecimal rate, int nper, BigDecimal pv, BigDecimal fv, int type, Map<Integer, BigDecimal> adjust) {
		fv = (fv == null ? BigDecimal.ZERO : fv);
		BigDecimal one = BigDecimal.ONE;
		BigDecimal reciprocal = one.divide(rate.add(one), 20, BigDecimal.ROUND_HALF_UP);
		BigDecimal result = pv.subtract(fv.multiply(reciprocal.pow(nper)));
		...
		BigDecimal div = BigDecimal.ZERO;
		for (int i = 0; i < nper; i++) {
			BigDecimal rentTemp = adjust.get(i + 1);
			if(rentTemp != null){
				result = result.subtract(rentTemp.multiply(reciprocal.pow(i + (type == 0 ? 0 : 1))));
			} else {
				div = div.add(reciprocal.pow(i + (type == 0 ? 0 : 1)));
...
//	}
	...
	public static BigDecimal getPMT(CalculateCondition condition, List<BigDecimal> adjust) {
		...
		return getPMT(issueRate, nper, pv, fv, type, adjust);
	}
	
	public static BigDecimal getPMT(CalculateCondition condition, Map<Integer, BigDecimal> adjust) {
