...
public class TransferInterestServiceImpl implements TransferInterestService {
	...
	@Override
	public String updateConditionByTransferInterest(String adjustId, String docId, String contractId,String adjusttype) throws Exception {
...
//		String hsql = "  FROM com.business.entity.interest.FundStandardInterest WHERE id = '"+adjustId+"' ";
//		List<FundStandardInterest> fis_l =  this.tableService.findResultsByHSQL(hsql);
//		if(fis_l.size() > 0){
//			fsi = fis_l.get(0);
//			logger.debug("央行调息表UUID:"+fsi.getId());
//		}
		...
		//调息处理入口
		re_ht = rtrsi.processTransRate(adjustId, contractInfo.getId(), contractInfo.getContractId(), null, docId,adjusttype);
		if ("true".equals(re_ht.get("isSucess"))) {
			if ("true".equals(re_ht.get("isOver"))) {
				...
				faict.setAdjustStyle(adjusttype);
