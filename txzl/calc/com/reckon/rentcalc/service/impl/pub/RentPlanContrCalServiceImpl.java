...
//		corpusList.set(0, corpus.toString());
	...
	public FundRentPlanBean rentPlanCalByDevi(ConditionBean cb, TabCalBean tcb, int startList, FundRentPlanBean frpb, String[] rentAdjustList) throws Exception {
		...
		InterestCalServiceImpl icsi = new InterestCalServiceImpl();
		List<String> interests = icsi.getInterestList(frpb.getRentList(), leas_income, cb.getCleanLeaseMoney(), frpb.getYearRateList(), cb.getPeriodType(), cb.getGrace(), cb.getIncomeNumberYear(), cb.getEquipEndValue(), rentAdjustList,cb.getRateAdjustType());
		frpb.setInterestBusinessList(interests);
		...
		// 调用保存的操作
		rpcd.addRentPlan(tcb.getRentPlan_tb(), frpb, tcb, startList,null);
	...
	public FundRentPlanBean getKnowingRentPlanForSpecial(ConditionBean cb, FundRentPlanBean frpb,SpecialRulesBean  specialRulesBean) throws Exception {
		List<String> rentList = new ArrayList<String>();
		List<String> yearRateList = new ArrayList<String>();
		Integer startlist = specialRulesBean.getStartList();//开始期项
		Integer endlist = specialRulesBean.getEndList();//截止期项
		BigDecimal boforeCorpus=cb.getRemainCorpus();
		String rent = MoneyUtils.getZeroStr(specialRulesBean.getRegularPlanmoney().setScale(RentTools.getRentAccuracy(), BigDecimal.ROUND_HALF_UP).toString());//对应租金值  
		...
		cb.setRemainCorpus(cb.getRemainCorpus());
		...
		// 加载利息列表  并返回剩余本金
		InterestCalServiceImpl icsi = new InterestCalServiceImpl();
		List<String> interests = icsi.getInterestList(rentList, cb.getCleanLeaseMoney(), cb, specialRulesBean, cb.getPeriodType(), cb.getGrace(), cb.getIncomeNumberYear(), cb.getEquipEndValue(),rateReCal);
		...
		for(String str:corpusList){
			if(Double.parseDouble(str)<0){
				throw new LeasingException("期数："+specialRulesBean.getStartList()+"到"+specialRulesBean.getEndList()+ "指定租金阶段： 指定的租金过小！");
			}
		}
		...
		cb.setRemainLeaseTerm(cb.getRemainLeaseTerm()-(endlist-startlist+1)*specialRulesBean.getRegularMonths());
		return frpb;
	}
	
	public FundRentPlanBean getKnowingCorpusPlanForSpecial(ConditionBean cb, FundRentPlanBean frpb,SpecialRulesBean  specialRulesBean) throws Exception {
		List<String> corpusList = new ArrayList<String>();
		List<String> yearRateList = new ArrayList<String>();
		Integer startlist = specialRulesBean.getStartList();//开始期项
		Integer endlist = specialRulesBean.getEndList();//截止期项
		BigDecimal boforeCorpus=cb.getRemainCorpus();
		String corpus = MoneyUtils.getZeroStr(specialRulesBean.getRegularPlanmoney().setScale(RentTools.getCorpusAccuracy(), BigDecimal.ROUND_HALF_UP).toString());//对应本金值  
		for (int j = startlist; j <= endlist; j++) {
		...
		// 加载利息列表 返回剩余本金
		InterestCalServiceImpl icsi = new InterestCalServiceImpl();
		List<String> interests = icsi.getKnowingCorpusInterestList(corpusList, cb.getCleanLeaseMoney(), cb, specialRulesBean, cb.getPeriodType(), cb.getGrace(), cb.getIncomeNumberYear(), cb.getEquipEndValue(),rateReCal);
		frpb.getInterestBusinessList().addAll(interests);
		
		//加载租金列表
		List<String> rentList = new ArrayList<String>();
		for (int j = startlist,i=0; j <= endlist; j++,i++) {
...
public FundRentPlanBean getEqualRentPlanForSpecial(ConditionBean cb, FundRentPlanBean frpb,SpecialRulesBean  specialRulesBean) throws Exception {
		...
		List<String> rentList = rsi.getRentListByCondForSpecial(cb,specialRulesBean);
		frpb.getRentList().addAll(rentList);
		// 加载利息列表
		InterestCalServiceImpl icsi = new InterestCalServiceImpl();
		List<String> interests = icsi.getInterestList(rentList, cb.getRemainCorpus().toString(), cb, specialRulesBean, cb.getPeriodType(), cb.getGrace(), cb.getIncomeNumberYear(), cb.getEquipEndValue(),rateReCal);
		frpb.getInterestBusinessList().addAll(interests);
		...
		for (int j = startlist,i=0; j <= endlist; j++,i++) {
			...
			totalcorpus=totalcorpus.add(new BigDecimal(rentList.get(i)).subtract(new BigDecimal(interests.get(i))));
		}
		frpb.getCorpusBusinessList().addAll(corpusList);
