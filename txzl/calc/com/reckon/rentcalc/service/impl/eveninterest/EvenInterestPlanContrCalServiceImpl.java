...
public class EvenInterestPlanContrCalServiceImpl extends RentPlanContrCalServiceImpl {
	...
	public FundRentPlanBean rentPlanCalByConditionAndTab(ConditionBean cb, TabCalBean tcb, int startList, FundRentPlanBean frpb,List<FundPutPlan> fpps) throws Exception {
		...
		frpb = calFina(cb, tcb, startList, frpb,fpps);//做财务计划测算
	 ...
	 * 
	 */
	public FundRentPlanBean calFina(ConditionBean cb, TabCalBean tcb, int startList, FundRentPlanBean frpb,List<FundPutPlan> fpps) throws Exception {
	...
	public FundRentPlanBean rentPlanCalByDevi(ConditionBean cb, TabCalBean tcb, String rentScale, AdjustBean adb, String[] rentAdjustList) throws Exception {
		...
		for (int i = 0; i < frpb.getYearRateList().size(); i++) {
			year_rate_list.add(RateTools.getPreRate(frpb.getYearRateList().get(i).toString(), cb.getIncomeNumberYear(),cb.getRateAdjustType()));
		}
		...
		// 调用保存的操作
		rpcd.addRentPlan(tcb.getRentPlan_tb(), frpb, tcb, adb.getStartList(),cb);
