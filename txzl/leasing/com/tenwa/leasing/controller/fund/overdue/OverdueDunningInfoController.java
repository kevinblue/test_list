...
public class OverdueDunningInfoController {
	...
	/**
	  * 批量添加修改信审部门、业务部门、行业部门插入到t_business_credit_config表中
	  * @param request
	  * @param response
	  * @return 返回响应信息
	  * @throws Exception
	  */ 
	@RequestMapping(value = "/saveMultiXinshenDunningInfo.acl")
	@ResponseBody
	public String saveMultiXinshenDunningInfo(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			Map<String,String> model =QueryUtil.getRequestParameterMapByAjax(request);
			this.baseService.saveMultiXinshenDunningInfo(model);
		}catch (Exception e) {
			return "修改失败!["+e.getMessage()+"]";
		}
		return null;
