	...
	public void createVoucherReceiveFund(ContractInfo contractInfo,String jsonFundString,FundEbankData fbd) throws Exception {
		...
		for(int i=0;i<jsonArray.length();i++){
			...
			if (map2F6!=null){
				...
				map1.put("voucherSummary", mapF5);
				map1.put("subjectsId", "108");// VOUCHERASS_STACTS_CONFIG表中SUBJECTS_CODE对应的SUBJECTS_ID
				map1.put("debitMoney", Tools.formatNumberDoubleTwo(sum));// 借方金额
				map1.put("lenderMoney", "0"); // 贷方金额
				// 租金红冲没有网银信息
				if (fbd == null) {
					map1.put("ebankFactDate", currentDate);// 网银到账日期
				} else {
					map1.put("ebankFactDate", fbd.getFactDate());// 网银到账日期
				}
				map1.put("accountDate", accountDate);// 自行指定日期情况下,可不传
				list.add(map1);
